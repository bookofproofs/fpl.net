...
type TestPredicates () =
    ...
    member this.TestPredicateWithArgs2 () =
        ...
        let actual = sprintf "%O" result
        let expected = """Success: PredicateWithArgs (Self [], [PredicateWithoutArgs (Var "i")])""".Trim().Replace("\r","")
        Assert.AreEqual(expected, actual);
  ...
  (Var "myOp", [PredicateWithArgs (AliasedId ["NeutralElement"], [])])""".Trim().Replace("\r","")
        Assert.AreEqual(expected, actual);
  ...
  (Var "myOp",
   [PredicateWithArgs (AliasedId ["NeutralElement"], []);
    PredicateWithArgs (AliasedId ["SomeProperty"], [])])""".Trim().Replace("\r","")
        Assert.AreEqual(expected, actual);
   ...
   (([BlockVariableDeclaration
      ...
      BlockStatement
        (Range
           ((Var "proceedingResult", Var "p"),
            [Assertion (PredicateWithoutArgs (Var "proceedingResult"))]))],
     Undefined), And [PredicateWithoutArgs (Var "p")]))""".Trim().Replace("\r","")
        Assert.AreEqual(expected, actual);
