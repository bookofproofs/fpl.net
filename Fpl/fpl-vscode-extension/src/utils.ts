...
export function removeDirectorySync(p: string): Promise<string> {
...
export function makeDirectory(p: string): Promise<string> {
    return new Promise((resolve, reject) => {
        fs.mkdir(p, err => {
            if (err) {
                reject(err.message);
...
export function downloadFile(url: string, dest: string, maxRedirects = 5): Promise<void> {
    return new Promise((resolve, reject) => {
        const doRequest = (reqUrl: string, redirectsLeft: number): void => {
...
export function dispatchRuntime(runtimeName: string): [string, string] {
...
export function installRuntime(runtimeName: string, downloadPath: string, fileUrlDir: string, fileUrlName: string): Promise<string> {
    return new Promise((resolve, reject) => {
...
export function acquireDotnetRuntime(runtimeName: string, relPathToDotnetRuntime: string): Promise<string> {
    return new Promise((resolve, reject) => {
        const dotnetExecutableName = process.platform === 'win32' ? 'dotnet.exe' : 'dotnet';
        const pathToDotNetExe = path.join(relPathToDotnetRuntime, dotnetExecutableName);
