...
open Fpl2Interpreter.SymbolTable.Types2.CompoundPredicates
open Fpl2Interpreter.SymbolTable.Types2.Variables
open Fpl2Interpreter.SymbolTable.Types2.References
open Fpl2Interpreter.SymbolTable.Types2.Definitions
open Fpl2Interpreter.SymbolTable.Types3.SelfParent
open Fpl2Interpreter.SymbolTable.Types3.IsOperator
open Fpl2Interpreter.SymbolTable.Types3.PredicativeBlocks
open Fpl2Interpreter.SymbolTable.Types3.DefinitionProperties
open Fpl2Interpreter.SymbolTable.Types3.Quantifiers
open Fpl2Interpreter.SymbolTable.Types3.RulesOfInferences
open Fpl2Interpreter.SymbolTable.Types3.Extensions
open Fpl2Interpreter.SymbolTable.Types3.Delegates
open Fpl2Interpreter.SymbolTable.Types3.MapCases
open Fpl2Interpreter.SymbolTable.Types3.Localization
open Fpl2Interpreter.SymbolTable.Types3.AssertStmt
open Fpl2Interpreter.SymbolTable.Types3.Assignments
open Fpl2Interpreter.SymbolTable.Types3.ForStmt
open Fpl2Interpreter.SymbolTable.Types3.CasesStmt
...
type Structure() =
        ...
        if infiniteLoop.Contains(root) then
            ...
            if identifier = "" then 
                if root.Name = firstTypeNode then 
                    Some root
                elif root.RefersTo.IsSome && root.RefersTo.Value.Name = firstTypeNode then 
                    root.RefersTo
                else
                ...
                if root.Name = firstTypeNode && searchItem = identifier then 
                    Some root
                elif root.RefersTo.IsSome && root.RefersTo.Value.Name = firstTypeNode && searchItem = identifier then 
                    root.RefersTo
                else
                    match root.Scope.Values |> Seq.tryPick (findNamedItem firstTypeNode identifier infiniteLoop) with 
                    | Some found -> Some found
    ...
    let getName nodeType = 
        ...
        | "FplIntrinsicInd" ->
            ...
            [x.Name; x.ShortName; x.FplId; x.TypeId; $"""{match x.RunOrder with Some _ -> "Some" | None -> PrimNone}"""; x.Represent(); x.Type SignatureType.Mixed]
        | "FplIntrinsicFalse" ->
            let x = new FplIntrinsicFalse(positions, parent)
            [x.Name; x.ShortName; x.FplId; x.TypeId; $"""{match x.RunOrder with Some _ -> "Some" | None -> PrimNone}"""; x.Represent(); x.Type SignatureType.Mixed]
        | "FplIntrinsicTpl" ->
            let x = new FplIntrinsicTpl(LiteralTpl, positions, parent)
            [x.Name; x.ShortName; x.FplId; x.TypeId; $"""{match x.RunOrder with Some _ -> "Some" | None -> PrimNone}"""; x.Represent(); x.Type SignatureType.Mixed]
        | "FplIntrinsicTrue" ->
            let x = new FplIntrinsicTrue(positions, parent)
            [x.Name; x.ShortName; x.FplId; x.TypeId; $"""{match x.RunOrder with Some _ -> "Some" | None -> PrimNone}"""; x.Represent(); x.Type SignatureType.Mixed]
        | "FplIntrinsicUndef" ->
        ...
        | "FplPredicate" ->
            ...
            x.Value <- Some (new FplIntrinsicTrue(positions, x))
            [x.Name; x.ShortName; x.FplId; x.TypeId; $"""{match x.RunOrder with Some _ -> "Some" | None -> PrimNone}"""; x.Represent(); x.Type SignatureType.Mixed]
        | "FplPredicateList" ->
            let x = new FplPredicateList(positions, parent, 0)
            x.ArgList.Add (new FplIntrinsicTrue(positions, parent))
            x.ArgList.Add (new FplIntrinsicTrue(positions, parent))
            [x.Name; x.ShortName; x.FplId; x.TypeId; $"""{match x.RunOrder with Some _ -> "Some" | None -> PrimNone}"""; x.Represent(); x.Type SignatureType.Mixed]
        | "FplProof" ->
    ...
    let testSkeleton nodeType filename fplCode identifier = 
        ...
        | Some (node:FplGenericNode) ->
            if node.Name = PrimRoot then 
                Assert.IsInstanceOfType(node, typeof<FplRoot>)
    ...
    [<DataRow("FplIntrinsicFalse")>]
    [<DataRow("FplIntrinsicTpl")>]
    [<DataRow("FplIntrinsicTrue")>]
    ...
    member this.TestBlockTypeName(var) =
        ...
        | "FplAssignment" ->
            Assert.AreEqual<string>(PrimAssignmentL, (getName var).[index])
        | "FplAxiom" ->
        ...
        | "FplCases" ->
            Assert.AreEqual<string>(PrimCasesL, (getName var).[index])
        | "FplCaseElse" ->
            Assert.AreEqual<string>(PrimCaseElseL, (getName var).[index])
        | "FplCaseSingle" ->
            Assert.AreEqual<string>(PrimCaseSingleL, (getName var).[index])
        | "FplClass" ->
        ...
        | "FplForInStmt" ->
            Assert.AreEqual<string>(PrimForInStmtL, (getName var).[index])
        | "FplForInStmtDomain" ->
            Assert.AreEqual<string>(PrimForInStmtDomainL, (getName var).[index])
        | "FplForInStmtEntity" ->
            Assert.AreEqual<string>(PrimForInStmtEntityL, (getName var).[index])
        | "FplFunctionalTerm" ->
        ...
        | "FplIntrinsicFalse" ->
            Assert.AreEqual<string>(PrimFalse, (getName var).[index])
        | "FplIntrinsicTpl" ->
            Assert.AreEqual<string>(PrimIntrinsicTpl, (getName var).[index])
        | "FplIntrinsicTrue" ->
            Assert.AreEqual<string>(PrimTrue, (getName var).[index])
        | "FplIntrinsicUndef" ->
        ...
        | "FplMapCaseElse" ->
            Assert.AreEqual<string>(PrimMapCaseElseL, (getName var).[index])        
        | "FplMapCaseSingle" ->
            Assert.AreEqual<string>(PrimMapCaseSingleL, (getName var).[index])
        | "FplMapCases" ->
            Assert.AreEqual<string>(PrimMapCasesL, (getName var).[index])        
        | "FplMapping" ->
        ...
        | "FplUndetermined" ->
            Assert.AreEqual<string>(LiteralUndetL, (getName var).[index])
        | "FplVariable" -> 
    ...
    member this.TestFplBlockTypeFplId(var) =
        ...
        | "FplBaseConstructorCall" ->
            Assert.AreEqual<string>(LiteralObj, (getName var).[index])
        | "FplCases" ->
            Assert.AreEqual<string>(LiteralCases, (getName var).[index])
        | "FplCaseElse" ->
        ...
        | "FplDisjunction" ->
            Assert.AreEqual<string>(LiteralOr, (getName var).[index])        
        | "FplEquality" ->
            Assert.AreEqual<string>($"{LiteralDel}{PrimDelegateEqual}", (getName var).[index])        
        | "FplEquivalence" ->
            Assert.AreEqual<string>(LiteralIif, (getName var).[index])        
        | "FplExclusiveOr" ->
            Assert.AreEqual<string>(LiteralXor, (getName var).[index])        
        ...
        | "FplIntrinsicInd" ->
            Assert.AreEqual<string>(LiteralInd, (getName var).[index])
        | "FplIntrinsicFalse" ->
            Assert.AreEqual<string>(LiteralFalse, (getName var).[index])
        | "FplIntrinsicTpl" ->
            Assert.AreEqual<string>(LiteralTpl, (getName var).[index])
        | "FplIntrinsicTrue" ->
            Assert.AreEqual<string>(LiteralTrue, (getName var).[index])
        ...
        | "FplMapCases" ->
            Assert.AreEqual<string>(LiteralMapCases, (getName var).[index])        
        | "FplMapping" ->
            Assert.AreEqual<string>(LiteralObj, (getName var).[index])
        | "FplNegation" ->
            Assert.AreEqual<string>(LiteralNot, (getName var).[index])
    ...
    member this.TestStructureFplArgInferenceDerived(nodeType, varVal, fplCode, identifier) =
        ...
        | "FplArgInferenceDerived", "00" -> 
            ...
            Assert.AreEqual<int>(0, parent.Scope.Count)
            Assert.IsInstanceOfType(node, typeof<FplArgInferenceDerived>)
            Assert.AreEqual<int>(1, node.ArgList.Count)
            Assert.AreEqual<int>(0, node.Scope.Count)
    ...
    member this.TestStructureFplArgInferenceRevoke(nodeType, varVal, fplCode, identifier) =
        ...
        | "FplArgInferenceRevoke", "00" -> 
            ...
            Assert.AreEqual<int>(1, parent.ArgList.Count) // no justification (therefore 1)
            Assert.AreEqual<int>(0, parent.Scope.Count)
            Assert.IsInstanceOfType(node, typeof<FplArgInferenceRevoke>)
            Assert.AreEqual<int>(0, node.ArgList.Count)
            Assert.AreEqual<int>(0, node.Scope.Count)
        | _ -> failwith($"unmatched test {nodeType} {varVal}")
    ...
    member this.TestStructureFplArgInferenceTrivial(nodeType, varVal, fplCode, identifier) =
        ...
        | "FplArgInferenceTrivial", "00" ->
            ...
            Assert.AreEqual<int>(0, parent.Scope.Count)
            Assert.IsInstanceOfType(node, typeof<FplArgInferenceTrivial>)
            Assert.AreEqual<int>(0, node.ArgList.Count)
            Assert.AreEqual<int>(0, node.Scope.Count)
        | _ -> failwith($"unmatched test {nodeType} {varVal}")
    ...
    member this.TestStructureFplArgument(nodeType, varVal, fplCode, identifier) =
        ...
        match nodeType, varVal with
        | "FplArgument", "00" ->
            Assert.IsInstanceOfType(parent, typeof<FplProof>)
            Assert.AreEqual<int>(0, parent.ArgList.Count)
            Assert.AreEqual<int>(1, parent.Scope.Count)
            Assert.IsInstanceOfType(node, typeof<FplArgument>)
    ...
    member this.TestStructureFplAssertion(nodeType, varVal, fplCode, identifier) =
        ...
        match nodeType, varVal with
        | "FplAssertion", "00" ->
            Assert.IsInstanceOfType(parent, typeof<FplConstructor>)
            Assert.AreEqual<int>(1, parent.ArgList.Count)
            Assert.AreEqual<int>(0, parent.Scope.Count)
            Assert.IsInstanceOfType(node, typeof<FplAssertion>)
            Assert.AreEqual<int>(1, node.ArgList.Count)
            Assert.AreEqual<int>(0, node.Scope.Count)
        | _ -> failwith($"unmatched test {nodeType} {varVal}")
    ...
    member this.TestStructureFplAssignment(nodeType, varVal, fplCode, identifier) =
        ...
        match nodeType, varVal with
        | "FplAssignment", "00" -> 
            Assert.IsInstanceOfType(parent, typeof<FplPredicate>)
            Assert.AreEqual<int>(2, parent.ArgList.Count)
            Assert.AreEqual<int>(1, parent.Scope.Count)
            Assert.IsInstanceOfType(node, typeof<FplAssignment>)
            Assert.AreEqual<int>(2, node.ArgList.Count) // reference to variable := reference to false
            Assert.AreEqual<int>(0, node.Scope.Count)
            let assignment = node :?> FplAssignment
            Assert.IsInstanceOfType(assignment.Assignee.Value, typeof<FplVariable>)
            Assert.IsInstanceOfType(assignment.AssignedValue.Value, typeof<FplIntrinsicFalse>)
        | _ -> failwith($"unmatched test {nodeType} {varVal}")
    ...
    member this.TestStructureFplAxiom(nodeType, varVal, fplCode, identifier) =
        ...
        | "FplAxiom", "00" -> 
            Assert.IsInstanceOfType(parent, typeof<FplTheory>)
            Assert.AreEqual<int>(0, parent.ArgList.Count)
            Assert.AreEqual<int>(1, parent.Scope.Count)
            Assert.IsInstanceOfType(node, typeof<FplAxiom>)
            Assert.AreEqual<int>(1, node.ArgList.Count)
            Assert.AreEqual<int>(0, node.Scope.Count)
        | _ -> failwith($"unmatched test {nodeType} {varVal}")
    ...
    member this.TestStructureFplBase(nodeType, varVal, fplCode, identifier) =
        ...
        match nodeType, varVal with
        | "FplBase", "00" ->
            Assert.IsInstanceOfType(parent, typeof<FplClass>)
            Assert.AreEqual<int>(2, parent.ArgList.Count)
            Assert.AreEqual<int>(1, parent.Scope.Count)
            Assert.IsInstanceOfType(node, typeof<FplBase>)
            Assert.AreEqual<int>(0, node.ArgList.Count)
            Assert.AreEqual<int>(0, node.Scope.Count)
        ...
        | "FplBase", "01" ->
            ...
            Assert.IsInstanceOfType(node.RefersTo.Value, typeof<FplClass>)
        | "FplBase", "02" ->
            Assert.IsInstanceOfType(parent, typeof<FplClass>)
            Assert.AreEqual<int>(2, parent.ArgList.Count)
            Assert.AreEqual<int>(1, parent.Scope.Count)
    ...
    member this.TestStructureFplBaseConstructorCall(nodeType, varVal, fplCode, identifier) =
        ...
        | "FplBaseConstructorCall", "00" -> 
            ...
            Assert.AreEqual<int>(0, parent.Scope.Count) 
            Assert.IsInstanceOfType(node, typeof<FplBaseConstructorCall>)
    ...
    member this.TestStructureFplCaseElse(nodeType, varVal, fplCode, identifier) =
        ...
        match nodeType, varVal with
        | "FplCaseElse", "00" ->
            Assert.IsInstanceOfType(parent, typeof<FplCases>)
            Assert.AreEqual<int>(4, parent.ArgList.Count)
            Assert.AreEqual<int>(0, parent.Scope.Count)
            Assert.IsInstanceOfType(node, typeof<FplCaseElse>)
    ...
    member this.TestStructureFplCases(nodeType, varVal, fplCode, identifier) =
        ...
        | "FplCases", "00" ->
            ...
            Assert.IsInstanceOfType(node, typeof<FplCases>)
    ...
    member this.TestStructureFplCaseSingle(nodeType, varVal, fplCode, identifier) =
        ...
        | "FplCaseSingle", "00" ->
            ...
            Assert.AreEqual<int>(4, parent.ArgList.Count)
            Assert.AreEqual<int>(0, parent.Scope.Count)
            Assert.IsInstanceOfType(node, typeof<FplCaseSingle>)
    ...
    member this.TestStructureFplClass(nodeType, varVal, fplCode, identifier) =
        ...
        | "FplClass", "00" -> 
            ...
            Assert.AreEqual<int>(0, parent.ArgList.Count)
            Assert.AreEqual<int>(1, parent.Scope.Count)
            Assert.IsInstanceOfType(node, typeof<FplClass>)
            Assert.AreEqual<int>(0, node.ArgList.Count)
            Assert.AreEqual<int>(1, node.Scope.Count)
        | "FplClass", "00a" -> 
            Assert.IsInstanceOfType(parent, typeof<FplTheory>)
    ...
    member this.TestStructureFplConjecture(nodeType, varVal, fplCode, identifier) =
        ...
        | "FplConjecture", "00" -> 
            ...
            Assert.AreEqual<int>(0, parent.ArgList.Count)
            Assert.AreEqual<int>(1, parent.Scope.Count)
            Assert.IsInstanceOfType(node, typeof<FplConjecture>)
            Assert.AreEqual<int>(1, node.ArgList.Count)
            Assert.AreEqual<int>(0, node.Scope.Count)
        | _ -> failwith($"unmatched test {nodeType} {varVal}")
    ...
    member this.TestStructureFplConjunction(nodeType, varVal, fplCode, identifier) =
        ...
        | "FplConjunction", "00" ->
            ...
            Assert.AreEqual<int>(1, parent.ArgList.Count)
            Assert.AreEqual<int>(0, parent.Scope.Count)
            Assert.IsInstanceOfType(node, typeof<FplConjunction>)
            Assert.AreEqual<int>(2, node.ArgList.Count)
            Assert.AreEqual<int>(0, node.Scope.Count)
        | _ -> failwith($"unmatched test {nodeType} {varVal}")
    ...
    member this.TestStructureFplConstructor(nodeType, varVal, fplCode, identifier) =
        ...
        | "FplConstructor", "00" -> 
            ...
            Assert.AreEqual<int>(1, parent.Scope.Count)
            Assert.IsInstanceOfType(node, typeof<FplConstructor>)
    ...
    member this.TestStructureFplCorollary(nodeType, varVal, fplCode, identifier) =
        ...
        | "FplCorollary", "00a" ->
            ...
            Assert.AreEqual<int>(0, parent.ArgList.Count)
            Assert.AreEqual<int>(1, parent.Scope.Count) 
            Assert.IsInstanceOfType(node, typeof<FplCorollary>) 
            Assert.AreEqual<int>(1, node.ArgList.Count)
            Assert.AreEqual<int>(0, node.Scope.Count)
        // corollary with wrong parent
        | "FplCorollary", "00b" 
        ...
        | "FplCorollary", "01b" ->
            ...
            Assert.AreEqual<int>(0, node.Scope.Count)
        | "FplCorollary", "01c" -> 
            Assert.IsInstanceOfType(parent, typeof<FplTheorem>)
            Assert.AreEqual<int>(1, parent.ArgList.Count)
            Assert.AreEqual<int>(1, parent.Scope.Count) 
            Assert.IsInstanceOfType(node, typeof<FplCorollary>)  
            Assert.AreEqual<int>(1, node.ArgList.Count)
        ...
        | "FplCorollary", "01d" -> 
            ...
            Assert.AreEqual<int>(0, node.Scope.Count)
        | "FplCorollary", "01e" -> 
            Assert.IsInstanceOfType(parent, typeof<FplProposition>)
            Assert.AreEqual<int>(1, parent.ArgList.Count)
            Assert.AreEqual<int>(1, parent.Scope.Count) 
            Assert.IsInstanceOfType(node, typeof<FplCorollary>)  
            Assert.AreEqual<int>(1, node.ArgList.Count)
            Assert.AreEqual<int>(0, node.Scope.Count)
        | "FplCorollary", "01f" -> 
            Assert.IsInstanceOfType(parent, typeof<FplCorollary>)
            Assert.AreEqual<int>(1, parent.ArgList.Count)
            Assert.AreEqual<int>(1, parent.Scope.Count) 
            Assert.IsInstanceOfType(node, typeof<FplCorollary>)  
            Assert.AreEqual<int>(1, node.ArgList.Count)
    ...
    member this.TestStructureFplDefaultConstructor(nodeType, varVal, fplCode, identifier) =
        ...
        | "FplDefaultConstructor", "00" ->
            ...
            Assert.AreEqual<int>(0, parent.ArgList.Count)
            Assert.AreEqual<int>(1, parent.Scope.Count)
            Assert.IsInstanceOfType(node, typeof<FplDefaultConstructor>)
            Assert.AreEqual<int>(0, node.ArgList.Count) 
            Assert.AreEqual<int>(0, node.Scope.Count)
    ...
    member this.TestStructureFplDisjunction(nodeType, varVal, fplCode, identifier) =
        ...
        | "FplDisjunction", "00" ->
            ...
            Assert.AreEqual<int>(1, parent.ArgList.Count)
            Assert.AreEqual<int>(0, parent.Scope.Count)
            Assert.IsInstanceOfType(node, typeof<FplDisjunction>)
            Assert.AreEqual<int>(2, node.ArgList.Count)
            Assert.AreEqual<int>(0, node.Scope.Count)
        | _ -> failwith($"unmatched test {nodeType} {varVal}")
    ...
    member this.TestStructureFplEquality(nodeType, varVal, fplCode, identifier) =
        ...
        | "FplEquality", "02" ->
            ...
            Assert.IsTrue(parent.RefersTo.IsSome)
            Assert.IsTrue(Object.ReferenceEquals(parent.RefersTo.Value, node))
            Assert.IsInstanceOfType(node, typeof<FplEquality>)
            Assert.AreEqual<int>(2, node.ArgList.Count)
            Assert.AreEqual<int>(0, node.Scope.Count)
        | _ -> failwith($"unmatched test {nodeType} {varVal}")
    ...
    member this.TestStructureFplEquivalence(nodeType, varVal, fplCode, identifier) =
        ...
        | "FplEquivalence", "00" ->
            ...
            Assert.AreEqual<int>(1, parent.ArgList.Count)
            Assert.AreEqual<int>(0, parent.Scope.Count)
            Assert.IsInstanceOfType(node, typeof<FplEquivalence>)
            Assert.AreEqual<int>(2, node.ArgList.Count)
            Assert.AreEqual<int>(0, node.Scope.Count)
        | _ -> failwith($"unmatched test {nodeType} {varVal}")
    ...
    member this.TestStructureFplExclusiveOr(nodeType, varVal, fplCode, identifier) =
        ...
        | "FplExclusiveOr", "00" ->
            ...
            Assert.AreEqual<int>(1, parent.ArgList.Count)
            Assert.AreEqual<int>(0, parent.Scope.Count)
            Assert.IsInstanceOfType(node, typeof<FplExclusiveOr>)
            Assert.AreEqual<int>(2, node.ArgList.Count)
            Assert.AreEqual<int>(0, node.Scope.Count)
        | _ -> failwith($"unmatched test {nodeType} {varVal}")
    ...
    member this.TestStructureFplExtension(nodeType, varVal, fplCode, identifier) =
        ...
        | "FplExtension", "00" -> 
            ...
            Assert.AreEqual<int>(0, parent.ArgList.Count)
            Assert.AreEqual<int>(1, parent.Scope.Count)
            Assert.IsInstanceOfType(node, typeof<FplExtension>)
            Assert.AreEqual<int>(2, node.ArgList.Count)
            Assert.AreEqual<int>(1, node.Scope.Count)
    ...
    member this.TestStructureFplExtensionObj(nodeType, varVal, fplCode, identifier) =
        ...
        | "FplExtensionObj", "00" ->
            ...
            Assert.IsInstanceOfType(node, typeof<FplExtensionObj>)
    ...
    [<DataRow("FplForInStmt", "01", """def cl Nat def func Sum(list:* Nat[ind])->Nat { dec a:obj result, addend: Nat result:=Zero() for addend in list { result:=Add(result,addend) }; return result }""", "")>]
    [<DataRow("FplForInStmt", "02", """def cl Nat def func Sum(from, to: Nat, arr:*Nat[Nat]) -> Nat { dec a:obj i, result: Nat result:=Zero() for i in ClosedRange(from,to) { result:=Add(result,arr[i]) }; return result }""", "")>]
    [<DataRow("FplForInStmt", "03", """def cl Nat def func Sum() -> Nat { dec addend, result: Nat result:=Zero() for addend in Nat { result:=Add(result,addend) }; return result }""", "")>]
    [<DataRow("FplForInStmt", "04", """def cl Nat def func Add(x,y:Nat)->Nat def func Sum()->Nat {dec addend, result: Nat for addend in Nat() { result:=Add(result,addend) }; ret result }""", "")>]
    [<TestMethod>]
    member this.TestStructureFplForInStmt(nodeType, varVal, fplCode, identifier) =
        ...
        match nodeType, varVal with
        | "FplForInStmt", "01" ->
            Assert.IsInstanceOfType(parent, typeof<FplFunctionalTerm>)
            Assert.AreEqual<int>(4, parent.ArgList.Count)
            Assert.AreEqual<int>(4, parent.Scope.Count)
            Assert.IsInstanceOfType(node, typeof<FplForInStmt>)
        ...
        | "FplForInStmt", "04" ->
            Assert.IsInstanceOfType(parent, typeof<FplFunctionalTerm>)
            Assert.AreEqual<int>(3, parent.ArgList.Count) // mapping / for stmt, return
            Assert.AreEqual<int>(2, parent.Scope.Count)
            Assert.IsInstanceOfType(node, typeof<FplForInStmt>)
            Assert.AreEqual<int>(3, node.ArgList.Count) // entity, domain, for body consisting of 1 stmt
            Assert.AreEqual<string>("addend", node.ArgList[0].Type SignatureType.Name)
    ...
    [<DataRow("FplForInStmtDomain", "01", """def cl Nat def func Sum(list:* Nat[ind])->Nat { dec a:obj result, addend: Nat result:=Zero() for addend in list { result:=Add(result,addend) }; return result }""", "")>]
    [<DataRow("FplForInStmtDomain", "02", """def cl Nat def func Sum(from, to: Nat, arr:*Nat[Nat]) -> Nat { dec a:obj i, result: Nat result:=Zero() for i in ClosedRange(from,to) { result:=Add(result,arr[i]) }; return result }""", "")>]
    [<DataRow("FplForInStmtDomain", "03", """def cl Nat def func Sum() -> Nat { dec addend, result: Nat result:=Zero() for addend in Nat { result:=Add(result,addend) }; return result }""", "")>]
    [<DataRow("FplForInStmtDomain", "04", """def cl Nat def func Add(x,y:Nat)->Nat def func Sum()->Nat {dec addend, result: Nat for addend in Nat() { result:=Add(result,addend) }; ret result }""", "")>]
    [<TestMethod>]
    member this.TestStructureFplForInStmtDomain(nodeType, varVal, fplCode, identifier) =
        ...
        match nodeType, varVal with
        | "FplForInStmtDomain", "01" ->
            Assert.IsInstanceOfType(parent, typeof<FplForInStmt>)
            Assert.AreEqual<int>(3, parent.ArgList.Count) // entity, domain, assignment
            Assert.AreEqual<int>(0, parent.Scope.Count)
            Assert.IsInstanceOfType(node, typeof<FplForInStmtDomain>)
            Assert.AreEqual<int>(0, node.ArgList.Count)
            Assert.AreEqual<int>(0, node.Scope.Count)
            Assert.IsTrue(node.RefersTo.IsSome)
            Assert.IsInstanceOfType(node.RefersTo.Value, typeof<FplVariableArray>)
        | "FplForInStmtDomain", "02" ->
            Assert.IsInstanceOfType(parent, typeof<FplForInStmt>)
            Assert.AreEqual<int>(3, parent.ArgList.Count) // entity, domain, assignment
            Assert.AreEqual<int>(0, parent.Scope.Count)
            Assert.IsInstanceOfType(node, typeof<FplForInStmtDomain>)
            ...
            Assert.AreEqual<int>(0, node.Scope.Count)
            Assert.IsTrue(node.RefersTo.IsSome)
            Assert.IsInstanceOfType(node.RefersTo.Value, typeof<FplReference>)
        | "FplForInStmtDomain", "03" ->
            Assert.IsInstanceOfType(parent, typeof<FplForInStmt>)
            Assert.AreEqual<int>(3, parent.ArgList.Count) // entity, domain, assignment
            Assert.AreEqual<int>(0, parent.Scope.Count)
            Assert.IsInstanceOfType(node, typeof<FplForInStmtDomain>)
    ...
    [<DataRow("FplForInStmtEntity", "01", """def cl Nat def func Sum(list:* Nat[ind])->Nat { dec a:obj result, addend: Nat result:=Zero() for addend in list { result:=Add(result,addend) }; return result }""", "")>]
    [<DataRow("FplForInStmtEntity", "02", """def cl Nat def func Sum(from, to: Nat, arr:*Nat[Nat]) -> Nat { dec a:obj i, result: Nat result:=Zero() for i in ClosedRange(from,to) { result:=Add(result,arr[i]) }; return result }""", "")>]
    [<DataRow("FplForInStmtEntity", "03", """def cl Nat def func Sum() -> Nat { dec addend, result: Nat result:=Zero() for addend in Nat { result:=Add(result,addend) }; return result }""", "")>]
    [<DataRow("FplForInStmtEntity", "04", """def cl Nat def func Add(x,y:Nat)->Nat def func Sum()->Nat {dec addend, result: Nat for addend in Nat() { result:=Add(result,addend) }; ret result }""", "")>]
    [<TestMethod>]
    member this.TestStructureFplForInStmtEntity(nodeType, varVal, fplCode, identifier) =
        ...
        match nodeType, varVal with
        | "FplForInStmtEntity", "04"
        | "FplForInStmtEntity", "03" 
        | "FplForInStmtEntity", "02" 
        | "FplForInStmtEntity", "01" ->
            Assert.IsInstanceOfType(parent, typeof<FplForInStmt>)
            Assert.AreEqual<int>(3, parent.ArgList.Count) // entity, domain, assignment
            Assert.AreEqual<int>(0, parent.Scope.Count)
            Assert.IsInstanceOfType(node, typeof<FplForInStmtEntity>)
            Assert.AreEqual<int>(0, node.ArgList.Count)
            Assert.AreEqual<int>(0, node.Scope.Count)
            Assert.IsTrue(node.RefersTo.IsSome)
            Assert.IsInstanceOfType(node.RefersTo.Value, typeof<FplVariable>)
        | _ -> failwith($"unmatched test {nodeType} {varVal}")
    ...
    member this.TestStructureFplFunctionalTerm(nodeType, varVal, fplCode, identifier) =
        ...
        | "FplFunctionalTerm", "00" -> 
            ...
            Assert.AreEqual<int>(0, parent.ArgList.Count)
            Assert.AreEqual<int>(1, parent.Scope.Count)
            Assert.IsInstanceOfType(node, typeof<FplFunctionalTerm>)
    ...
    // intrinsic constant
    [<DataRow("FplFunctionalTerm", "MF1", """def func T()->ind""", "")>]
    // non-intrinsic constant
    [<DataRow("FplFunctionalTerm", "MF2", """def func T()->ind {return $1}""", "")>]
    // intrinsic constant array
    [<DataRow("FplFunctionalTerm", "MF3", """def func T()->*ind[ind]""", "")>]
    // non-intrinsic constant array
    [<DataRow("FplFunctionalTerm", "MF4", """def func T()->*ind[ind] {dec x:*ind[ind] x[$1]:=$0 x[$2]:=$42; return x}""", "")>]
    [<TestMethod>]
    member this.TestStructureFplFunctionalTermIndexMapping(nodeType, varVal, fplCode, identifier) =
        let filename = "TestStructureFplFunctionalTermIndexMapping.fpl"
        let parent, node = testSkeleton nodeType filename fplCode identifier
    ...
    // intrinsic constant
    [<DataRow("FplFunctionalTerm", "MF1", """def func T()->obj""", "")>]
    // non-intrinsic constant
    [<DataRow("FplFunctionalTerm", "MF2", """def cl A def func T()->obj {return A()}""", "")>]
    // intrinsic constant array
    [<DataRow("FplFunctionalTerm", "MF3", """def func T()->*obj[ind]""", "")>]
    // non-intrinsic constant array
    [<DataRow("FplFunctionalTerm", "MF4", """def cl A def cl B def func T()->*obj[ind] {dec x:*obj[ind] x[$1]:=A() x[$2]:=B(); return x}""", "")>]
    [<TestMethod>]
    member this.TestStructureFplFunctionalTermObjectMapping(nodeType, varVal, fplCode, identifier) =
        let filename = "TestStructureFplFunctionalTermObjectMapping.fpl"
        let parent, node = testSkeleton nodeType filename fplCode identifier
    ...
    // intrinsic constant
    [<DataRow("FplFunctionalTerm", "MF1", """def func T()->tpl""", "")>]
    // non-intrinsic constant
    [<DataRow("FplFunctionalTerm", "MF2", """def cl A def func T()->tpl {return A()}""", "")>]
    // intrinsic constant array
    ...
    member this.TestStructureFplFunctionalTermTemplateMapping(nodeType, varVal, fplCode, identifier) =
        ...
        | "FplFunctionalTerm", "MF1" -> 
            ...
            Assert.IsInstanceOfType(node, typeof<FplFunctionalTerm>) 
            Assert.AreEqual<int>(1, node.ArgList.Count)  // mapping
            Assert.AreEqual<int>(1, node.Scope.Count) // template
            let map = (getMapping node).Value :?> FplMapping 
            match map.RefersTo with
            | None -> Assert.AreEqual<string>("no class", "no class")
            | Some _ -> Assert.IsTrue(false, "The is no to be returned class")
        ...
        | "FplFunctionalTerm", "MF4" -> 
            ...
            Assert.AreEqual<int>(2, node.Scope.Count) // one variable, one template
            let map = (getMapping node).Value :?> FplMapping 
            match map.RefersTo with
            | None -> Assert.AreEqual<string>("no class", "no class")
            | Some _ -> Assert.IsTrue(false, "The is no to be returned class")
    ...
    member this.TestStructureFplFunctionalTermPredMapping(nodeType, varVal, fplCode, identifier) =
        ...
        | "FplFunctionalTerm", "MF2a" -> 
            ...
            let nodeWithValue = node :?> FplGenericHasValue
            Assert.IsTrue(nodeWithValue.Value.IsSome)
            Assert.AreEqual<string>(LiteralUndet, node.Represent())
            let fn = node :?> FplFunctionalTerm
            Assert.AreEqual<string>("", fn.ConstantName) // missing, since non-intrinsic
    ...
    member this.TestStructureFplImplication(nodeType, varVal, fplCode, identifier) =
        ...
        | "FplImplication", "00" ->
            ...
            Assert.AreEqual<int>(1, parent.ArgList.Count)
            Assert.AreEqual<int>(0, parent.Scope.Count)
            Assert.IsInstanceOfType(node, typeof<FplImplication>)
            Assert.AreEqual<int>(2, node.ArgList.Count)
            Assert.AreEqual<int>(0, node.Scope.Count)
        | _ -> failwith($"unmatched test {nodeType} {varVal}")
    ...
    member this.TestStructureFplInstance(nodeType, varVal, fplCode, identifier) =
        ...
        match nodeType, varVal with
        | "FplInstance", "00" ->
            Assert.IsInstanceOfType(parent, typeof<FplAssignment>)
            Assert.AreEqual<int>(2, parent.ArgList.Count)
            Assert.AreEqual<int>(0, parent.Scope.Count)
            Assert.IsInstanceOfType(node, typeof<FplInstance>)
            Assert.AreEqual<int>(0, node.ArgList.Count)
            Assert.AreEqual<int>(0, node.Scope.Count)
            Assert.AreEqual<string>("""A()""", node.Represent())
        | _ -> failwith($"unmatched test {nodeType} {varVal}")
    ...
    member this.TestStructureFplIntrinsicInd(nodeType, varVal, fplCode, identifier) =
        ...
        | "FplIntrinsicInd", "00" ->
            ...
            Assert.IsTrue(parent.RefersTo.IsSome)
            Assert.IsTrue(Object.ReferenceEquals(node, (parent.RefersTo.Value)))
            Assert.IsInstanceOfType(node, typeof<FplIntrinsicInd>)
            Assert.AreEqual<int>(0, node.ArgList.Count)
            Assert.AreEqual<int>(0, node.Scope.Count)
        | _ -> failwith($"unmatched test {nodeType} {varVal}")

    [<DataRow("FplIntrinsicFalse", "01", """ax A {false}""", "")>]
    // reference to intrinsic false
    [<DataRow("FplIntrinsicFalse", "03c", """def pred A() {dec x:pred x:=false; x}""", "false")>]
    [<DataRow("FplIntrinsicFalse", "03d", """def pred A() {D(false)}""", "false")>]
    [<TestMethod>]
    member this.TestStructureFplIntrinsicFalse(nodeType, varVal, fplCode, identifier) =
        let filename = "TestStructureFplIntrinsicFalse.fpl"
        let parent, node = testSkeleton nodeType filename fplCode identifier
        
        match nodeType, varVal with
        | "FplIntrinsicFalse", "01" ->
            Assert.IsInstanceOfType(parent, typeof<FplReference>)
            Assert.AreEqual<int>(0, parent.ArgList.Count)
            Assert.AreEqual<int>(0, parent.Scope.Count)
            Assert.IsTrue(parent.RefersTo.IsSome)
            Assert.IsTrue(Object.ReferenceEquals(node, (parent.RefersTo.Value)))
            Assert.IsInstanceOfType(node, typeof<FplIntrinsicFalse>)
            Assert.AreEqual<int>(0, node.ArgList.Count)
            Assert.AreEqual<int>(0, node.Scope.Count)
        | "FplIntrinsicFalse", "03c" ->
            Assert.IsInstanceOfType(parent, typeof<FplReference>)
            Assert.AreEqual<int>(0, parent.ArgList.Count)
            Assert.AreEqual<int>(0, parent.Scope.Count)
            Assert.IsTrue(parent.RefersTo.IsSome)
    ...
    [<DataRow("FplIntrinsicTrue", "00", """ax A {true}""", "")>]
    // reference to intrinsic true
    [<DataRow("FplIntrinsicTrue", "03a", """def pred A() {dec x:pred x:=true; x}""", "true")>]
    [<DataRow("FplIntrinsicTrue", "03b", """def pred A() {D(true)}""", "true")>]
    [<TestMethod>]
    member this.TestStructureFplIntrinsicTrue(nodeType, varVal, fplCode, identifier) =
        let filename = "TestStructureFplIntrinsicTrue.fpl"
        let parent, node = testSkeleton nodeType filename fplCode identifier
        ...
        | "FplIntrinsicTrue", "00" ->
            ...
            Assert.IsTrue(parent.RefersTo.IsSome)
            Assert.IsTrue(Object.ReferenceEquals(node, (parent.RefersTo.Value)))
            Assert.IsInstanceOfType(node, typeof<FplIntrinsicTrue>)
            Assert.AreEqual<int>(0, node.ArgList.Count)
            Assert.AreEqual<int>(0, node.Scope.Count)
        | "FplIntrinsicTrue", "03a" ->
            Assert.IsInstanceOfType(parent, typeof<FplReference>)
            Assert.AreEqual<int>(0, parent.ArgList.Count)
            Assert.AreEqual<int>(0, parent.Scope.Count)
            Assert.IsTrue(parent.RefersTo.IsSome)
    ...
    [<DataRow("FplIntrinsicTpl", "00", """def pred TestId(x:tplTest) {true}""", "")>]
    [<DataRow("FplIntrinsicTpl", "01", """def func T()->tpl""", "")>]
    [<DataRow("FplIntrinsicTpl", "02", """def func T()->*tpl[ind]""", "")>]
    [<DataRow("FplIntrinsicTpl", "03", """def cl T {ctor T(x:tpl) {}}""", "")>]
    [<DataRow("FplIntrinsicTpl", "04", """def pred NotEqual (x,y: tpl) { not (x = y) }""", "")>]
    [<DataRow("FplIntrinsicTpl", "05", """def pred NotEqual (x,y: tpl) { dec u,w: tpl1; not (x = y) }""", "")>]
    ...
    member this.TestStructureFplIntrinsicTpl(nodeType, varVal, fplCode, identifier) =
        ...
        | "FplIntrinsicTpl", "00" ->
            ...
            Assert.AreEqual<int>(1, parent.ArgList.Count)
            Assert.AreEqual<int>(2, parent.Scope.Count) 
            Assert.IsInstanceOfType(node, typeof<FplIntrinsicTpl>)
            Assert.AreEqual<int>(0, node.ArgList.Count)
            Assert.AreEqual<int>(0, node.Scope.Count)
        | "FplIntrinsicTpl", "01" ->
            Assert.IsInstanceOfType(parent, typeof<FplFunctionalTerm>)
        ...
        | "FplIntrinsicTpl", "03" ->
            Assert.IsInstanceOfType(parent, typeof<FplClass>)
            Assert.AreEqual<int>(0, parent.ArgList.Count)
            Assert.AreEqual<int>(2, parent.Scope.Count) // constructor + template
            Assert.IsInstanceOfType(node, typeof<FplIntrinsicTpl>)
            Assert.AreEqual<int>(0, node.ArgList.Count)
            Assert.AreEqual<int>(0, node.Scope.Count)
        | "FplIntrinsicTpl", "04" ->
            Assert.IsInstanceOfType(parent, typeof<FplPredicate>)
            Assert.AreEqual<int>(1, parent.ArgList.Count)
            Assert.AreEqual<int>(3, parent.Scope.Count) // 2 vars, 1 template
            Assert.IsInstanceOfType(node, typeof<FplIntrinsicTpl>)
            Assert.AreEqual<int>(0, node.ArgList.Count)
            Assert.AreEqual<int>(0, node.Scope.Count)
        | "FplIntrinsicTpl", "05" ->
            Assert.IsInstanceOfType(parent, typeof<FplPredicate>)
            Assert.AreEqual<int>(1, parent.ArgList.Count)
            Assert.AreEqual<int>(6, parent.Scope.Count) // 4 vars, 2 templates
            Assert.IsInstanceOfType(node, typeof<FplIntrinsicTpl>)
            Assert.AreEqual<int>(0, node.ArgList.Count)
            Assert.AreEqual<int>(0, node.Scope.Count)
        | _ -> failwith($"unmatched test {nodeType} {varVal}")
    ...
    member this.TestStructureFplIntrinsicUndef(nodeType, varVal, fplCode, identifier) =
        ...
        | "FplIntrinsicUndef", "00" ->
            ...
            Assert.IsTrue(parent.RefersTo.IsSome)
            Assert.IsTrue(Object.ReferenceEquals(node, (parent.RefersTo.Value)))
            Assert.IsInstanceOfType(node, typeof<FplIntrinsicUndef>)
            Assert.AreEqual<int>(0, node.ArgList.Count)
            Assert.AreEqual<int>(0, node.Scope.Count)
        | _ -> failwith($"unmatched test {nodeType} {varVal}")
    ...
    member this.TestStructureFplIsOperator(nodeType, varVal, fplCode, identifier) =
        ...
        | "FplIsOperator", "00" ->
            ...
            Assert.AreEqual<int>(1, parent.ArgList.Count)
            Assert.AreEqual<int>(0, parent.Scope.Count)
            Assert.IsInstanceOfType(node, typeof<FplIsOperator>)
            Assert.AreEqual<int>(2, node.ArgList.Count)
            Assert.AreEqual<int>(0, node.Scope.Count)
        | _ -> failwith($"unmatched test {nodeType} {varVal}")
    ...
    member this.TestStructureFplJustification(nodeType, varVal, fplCode, identifier) =
        ...
        | "FplJustification", "08" ->
            ...
            Assert.AreEqual<int>(2, parent.ArgList.Count) 
            Assert.AreEqual<int>(0, parent.Scope.Count)
            Assert.IsInstanceOfType(node, typeof<FplJustification>)
    ...
    member this.TestStructureFplJustificationItemByAx(nodeType, varVal, fplCode, identifier) =
        ...
        | "FplJustificationItemByAx", "00" ->
            Assert.IsInstanceOfType(parent, typeof<FplJustification>)
            Assert.AreEqual<int>(1, parent.ArgList.Count) 
            Assert.AreEqual<int>(0, parent.Scope.Count)
            Assert.IsInstanceOfType(node, typeof<FplJustificationItemByAx>)
            Assert.AreEqual<int>(0, node.ArgList.Count)  
            Assert.AreEqual<int>(0, node.Scope.Count)
        | "FplJustificationItemByAx", "00a" ->
            ...
            Assert.AreEqual<int>(0, node.Scope.Count) 
            Assert.IsTrue(node.RefersTo.IsSome) 
            Assert.IsInstanceOfType(node.RefersTo.Value, typeof<FplAxiom>) // referenced axiom
    ...
    member this.TestStructureFplJustificationItemByConj(nodeType, varVal, fplCode, identifier) =
        ...
        | "FplJustificationItemByConj", "00a" ->
            ...
            Assert.AreEqual<int>(2, parent.ArgList.Count)
            Assert.AreEqual<int>(0, parent.Scope.Count)
            Assert.IsInstanceOfType(node, typeof<FplJustificationItemByConj>)
            Assert.AreEqual<int>(0, node.ArgList.Count)
            Assert.IsTrue(node.RefersTo.IsSome)
            Assert.IsInstanceOfType(node.RefersTo.Value, typeof<FplConjecture>)
    ...
    [<DataRow("FplJustificationItemByCor", "00", """cor A$1 {true} proof T$1 { 100. bycor A$1 |- false }""", "")>]
    [<DataRow("FplJustificationItemByCor", "01", """cor A$1 {true} proof T$1 { 100. bycor ATypo$1 |- false }""", "")>]
    [<TestMethod>]
    member this.TestStructureFplJustificationItemByCor(nodeType, varVal, fplCode, identifier) =
        ...
        match nodeType, varVal with
        | "FplJustificationItemByCor", "00" ->
            Assert.IsInstanceOfType(parent, typeof<FplJustification>)
            Assert.AreEqual<int>(1, parent.ArgList.Count)
            Assert.AreEqual<int>(0, parent.Scope.Count)
            Assert.IsInstanceOfType(node, typeof<FplJustificationItemByCor>)
            Assert.AreEqual<int>(0, node.ArgList.Count)
            Assert.AreEqual<int>(0, node.Scope.Count)
            Assert.IsTrue(node.RefersTo.IsSome)
            Assert.IsInstanceOfType(node.RefersTo.Value, typeof<FplCorollary>)
        | "FplJustificationItemByCor", "01" ->
            Assert.IsInstanceOfType(parent, typeof<FplJustification>)
            Assert.AreEqual<int>(1, parent.ArgList.Count)
            Assert.AreEqual<int>(0, parent.Scope.Count)
            Assert.IsInstanceOfType(node, typeof<FplJustificationItemByCor>)
    ...
    member this.TestStructureFplJustificationItemByDef(nodeType, varVal, fplCode, identifier) =
        ...
        | "FplJustificationItemByDef", "00f" ->
            ...
            Assert.AreEqual<int>(2, parent.ArgList.Count)
            Assert.AreEqual<int>(0, parent.Scope.Count)
            Assert.IsInstanceOfType(node, typeof<FplJustificationItemByDef>)
            Assert.AreEqual<int>(0, node.ArgList.Count)
            Assert.AreEqual<int>(0, node.Scope.Count)
            Assert.IsTrue(node.RefersTo.IsNone)
        ...
        | "FplJustificationItemByDef", "00h" ->
            ...
            Assert.AreEqual<int>(0, node.Scope.Count)
            Assert.IsTrue(node.RefersTo.IsSome)
            Assert.IsInstanceOfType(node.RefersTo.Value, typeof<FplPredicate>)
        | "FplJustificationItemByDef", "00i" ->
            ...
            Assert.AreEqual<int>(0, node.Scope.Count)
            Assert.IsTrue(node.RefersTo.IsSome)
            Assert.IsInstanceOfType(node.RefersTo.Value, typeof<FplFunctionalTerm>)
        | _ -> failwith($"unmatched test {nodeType} {varVal}")

    [<DataRow("FplJustificationItemByDefVar", "00", """def cl A thm T {dec x:A; true} proof T$1 { 100. bydef x |- is(x, A) }""", "")>]
    [<DataRow("FplJustificationItemByDefVar", "01", """def cl A thm T {true} proof T$1 {dec x:A; 100. bydef x |- is(x, A) }""", "")>]
    [<TestMethod>]
    member this.TestStructureFplJustificationItemByDefVar(nodeType, varVal, fplCode, identifier) =
        ...
        match nodeType, varVal with
        | "FplJustificationItemByDefVar", "00" 
        | "FplJustificationItemByDefVar", "01" ->
            Assert.IsInstanceOfType(parent, typeof<FplJustification>)
            Assert.AreEqual<int>(1, parent.ArgList.Count) // bydef x
            Assert.AreEqual<int>(0, parent.Scope.Count)
            Assert.IsInstanceOfType(node, typeof<FplJustificationItemByDefVar>)
            Assert.AreEqual<int>(0, node.ArgList.Count)
            Assert.AreEqual<int>(0, node.Scope.Count)
            Assert.IsTrue(node.RefersTo.IsSome)
            Assert.IsInstanceOfType(node.RefersTo.Value, typeof<FplVariable>)
    ...
    member this.TestStructureFplJustificationItemByInf(nodeType, varVal, fplCode, identifier) =
        ...
        | "FplJustificationItemByInf", "00a" ->
            ...
            Assert.AreEqual<int>(2, parent.ArgList.Count)
            Assert.AreEqual<int>(0, parent.Scope.Count)
            Assert.IsInstanceOfType(node, typeof<FplJustificationItemByInf>)
            Assert.AreEqual<int>(0, node.ArgList.Count)
            Assert.AreEqual<int>(0, node.Scope.Count)
            Assert.IsTrue(node.RefersTo.IsSome)
            Assert.IsInstanceOfType(node.RefersTo.Value, typeof<FplRuleOfInference>)
    ...
    member this.TestStructureFplJustificationItemByProofArgument(nodeType, varVal, fplCode, identifier) =
        ...
        | "FplJustificationItemByProofArgument", "00" ->
            ...
            Assert.AreEqual<int>(1, parent.ArgList.Count)
            Assert.AreEqual<int>(0, parent.Scope.Count)
            Assert.IsInstanceOfType(node, typeof<FplJustificationItemByProofArgument>)
            Assert.AreEqual<int>(0, node.ArgList.Count)
            Assert.AreEqual<int>(0, node.Scope.Count)
    ...
    member this.TestStructureFplJustificationItemByRefArgument(nodeType, varVal, fplCode, identifier) =
        ...
        | "FplJustificationItemByRefArgument", "00" ->
            ...
            Assert.AreEqual<int>(1, parent.ArgList.Count)
            Assert.AreEqual<int>(0, parent.Scope.Count)
            Assert.IsInstanceOfType(node, typeof<FplJustificationItemByRefArgument>)
            Assert.AreEqual<int>(0, node.ArgList.Count)
            Assert.AreEqual<int>(0, node.Scope.Count)
    ...
    member this.TestStructureFplJustificationItemByTheoremLikeStmt(nodeType, varVal, fplCode, identifier) =
        ...
        | "FplJustificationItemByTheoremLikeStmt", "00a" ->
            ...
            Assert.AreEqual<int>(2, parent.ArgList.Count)
            Assert.AreEqual<int>(0, parent.Scope.Count)
            Assert.IsInstanceOfType(node, typeof<FplJustificationItemByTheoremLikeStmt>)
            Assert.AreEqual<int>(0, node.ArgList.Count)
            Assert.AreEqual<int>(0, node.Scope.Count)
            Assert.IsTrue(node.RefersTo.IsSome)
            Assert.IsInstanceOfType(node.RefersTo.Value, typeof<FplTheorem>)
        | "FplJustificationItemByTheoremLikeStmt", "00b" ->
            ...
            Assert.AreEqual<int>(0, node.ArgList.Count)
            Assert.IsTrue(node.RefersTo.IsSome)
            Assert.IsInstanceOfType(node.RefersTo.Value, typeof<FplProposition>)
        | "FplJustificationItemByTheoremLikeStmt", "00c" ->
            ...
            Assert.AreEqual<int>(0, node.ArgList.Count)
            Assert.IsTrue(node.RefersTo.IsSome)
            Assert.IsInstanceOfType(node.RefersTo.Value, typeof<FplLemma>)
    ...
    [<DataRow("FplLanguage", "01", """loc not x := !tex: "\neg(" x ")" !eng: "not " x !ger: "nicht " x;""", "")>]
    [<DataRow("FplLanguage", "02", """loc and (x,y) := !eng: x " and " y !ger: x " und " y;""", "")>]
    [<TestMethod>]
    member this.TestStructureFplLanguage(nodeType, varVal, fplCode, identifier) =
        ...
        match nodeType, varVal with
        | "FplLanguage", "01" ->
            Assert.IsInstanceOfType(parent, typeof<FplLocalization>)
            Assert.AreEqual<int>(1, parent.ArgList.Count) // not x
            Assert.AreEqual<int>(4, parent.Scope.Count) // one variable and three translations
            Assert.IsInstanceOfType(node, typeof<FplLanguage>) // first tex language
            Assert.AreEqual<int>(3, node.ArgList.Count) // three translations
            Assert.AreEqual<int>(0, node.Scope.Count)
        | "FplLanguage", "02" ->
            Assert.IsInstanceOfType(parent, typeof<FplLocalization>)
            Assert.AreEqual<int>(1, parent.ArgList.Count) // and x y
            Assert.AreEqual<int>(4, parent.Scope.Count) // two variables and two translations
            Assert.IsInstanceOfType(node, typeof<FplLanguage>) // ger translation
            Assert.AreEqual<int>(3, node.ArgList.Count) // three translations
            Assert.AreEqual<int>(0, node.Scope.Count)
        | _ -> failwith($"unmatched test {nodeType} {varVal}")
    ...
    member this.TestStructureFplLemma(nodeType, varVal, fplCode, identifier) =
        ...
        | "FplLemma", "00" -> 
            ...
            Assert.AreEqual<int>(0, parent.ArgList.Count)
            Assert.AreEqual<int>(1, parent.Scope.Count)
            Assert.IsInstanceOfType(node, typeof<FplLemma>)
            Assert.AreEqual<int>(1, node.ArgList.Count)
            Assert.AreEqual<int>(0, node.Scope.Count)
        | "FplLemma", "01" -> 
            Assert.IsInstanceOfType(parent, typeof<FplTheory>)
    ...
    member this.TestStructureFplLocalization(nodeType, varVal, fplCode, identifier) =
        ...
        | "FplLocalization", "01" ->
            ...
            Assert.AreEqual<int>(0, parent.ArgList.Count)
            Assert.AreEqual<int>(1, parent.Scope.Count)
            Assert.IsInstanceOfType(node, typeof<FplLocalization>)
    ...
    member this.TestStructureFplMandatoryFunctionalTerm(nodeType, varVal, fplCode, identifier) =
        ...
        | "FplMandatoryFunctionalTerm", "00" -> 
            Assert.IsInstanceOfType(parent, typeof<FplPredicate>) // predicate parent
            Assert.AreEqual<int>(0, parent.ArgList.Count)
            Assert.AreEqual<int>(1, parent.Scope.Count)
            Assert.IsInstanceOfType(node, typeof<FplMandatoryFunctionalTerm>)
            Assert.AreEqual<int>(1, node.ArgList.Count) // intrinsic with mapping 
            Assert.AreEqual<int>(1, node.Scope.Count) // one variable
        | "FplMandatoryFunctionalTerm", "01" -> 
            Assert.IsInstanceOfType(parent, typeof<FplFunctionalTerm>) // functional term parent
        ...
        | "FplMandatoryFunctionalTerm", "02" -> 
            Assert.IsInstanceOfType(parent, typeof<FplClass>) // class parent
            Assert.AreEqual<int>(0, parent.ArgList.Count) 
            Assert.AreEqual<int>(2, parent.Scope.Count)
            Assert.IsInstanceOfType(node, typeof<FplMandatoryFunctionalTerm>)
            Assert.AreEqual<int>(1, node.ArgList.Count) // intrinsic with mapping 
    ...
    member this.TestStructureFplMandatoryPredicate(nodeType, varVal, fplCode, identifier) =
        ...
        | "FplMandatoryPredicate", "00" -> 
            ...
            Assert.AreEqual<int>(0, parent.ArgList.Count)
            Assert.AreEqual<int>(1, parent.Scope.Count)
            Assert.IsInstanceOfType(node, typeof<FplMandatoryPredicate>)
    ...
    member this.TestStructureFplMapCaseElse(nodeType, varVal, fplCode, identifier) =
        ...
        match nodeType, varVal with
        | "FplMapCaseElse", "00" ->
            Assert.IsInstanceOfType(parent, typeof<FplMapCases>)
            Assert.AreEqual<int>(4, parent.ArgList.Count)
            Assert.AreEqual<int>(0, parent.Scope.Count)
            Assert.IsInstanceOfType(node, typeof<FplMapCaseElse>)
            Assert.AreEqual<int>(1, node.ArgList.Count) 
            Assert.AreEqual<int>(0, node.Scope.Count)
        | "FplMapCaseElse", "01" ->
            Assert.IsInstanceOfType(parent, typeof<FplMapCases>)
    ...
    member this.TestStructureFplMapCases(nodeType, varVal, fplCode, identifier) =
        ...
        | "FplMapCases", "00" ->
            ...
            Assert.AreEqual<int>(0, parent.Scope.Count)
            Assert.IsInstanceOfType(node, typeof<FplMapCases>)
        ...
        | "FplMapCases", "01" ->
            ...
            Assert.AreEqual<int>(0, node.Scope.Count)
        | "FplMapCases", "02" ->
            Assert.IsInstanceOfType(parent, typeof<FplReturn>)
            Assert.AreEqual<int>(1, parent.ArgList.Count) 
            Assert.AreEqual<int>(0, parent.Scope.Count)
            Assert.IsInstanceOfType(node, typeof<FplMapCases>)
            Assert.AreEqual<int>(3, node.ArgList.Count) // 2 cases + 1 else
    ...
    member this.TestStructureFplMapCaseSingle(nodeType, varVal, fplCode, identifier) =
        ...
        | "FplMapCaseSingle", "00" ->
            ...
            Assert.AreEqual<int>(4, parent.ArgList.Count)
            Assert.AreEqual<int>(0, parent.Scope.Count)
            Assert.IsInstanceOfType(node, typeof<FplMapCaseSingle>)
            Assert.AreEqual<int>(2, node.ArgList.Count) 
            Assert.AreEqual<int>(0, node.Scope.Count)
        | "FplMapCaseSingle", "01" ->
            Assert.IsInstanceOfType(parent, typeof<FplMapCases>)
    ...
    [<DataRow("FplMapping", "MF4", """def func T()->pred""", "")>]
    [<DataRow("FplMapping", "MF4a", """def func T()->*pred[ind]""", "")>]
    [<DataRow("FplMapping", "MF4b", """def func T()->pred(x:obj)""", "")>]
    [<DataRow("FplMapping", "MF4c", """def func T()->*pred(x:obj)[ind]""", "")>]
    [<DataRow("FplMapping", "MF5", """def func T()->func""", "")>]
    [<DataRow("FplMapping", "MF5a", """def func T()->*func[ind]""", "")>]
    [<DataRow("FplMapping", "MF5b", """def func T()->func(x:obj)->obj""", "")>]
    [<DataRow("FplMapping", "MF5c", """def func T()->*func(x:obj)->obj[ind]""", "")>]
    [<DataRow("FplMapping", "MF6", """def func T()->tpl""", "")>]
    [<DataRow("FplMapping", "MF6a", """def func T()->*tpl[ind]""", "")>]
    ...
    member this.TestStructureFplMapping(nodeType, varVal, fplCode, identifier) =
        ...
        | "FplMapping", "00a" -> 
            ...
            Assert.IsInstanceOfType(node, typeof<FplMapping>)
            Assert.AreEqual<int>(0, node.ArgList.Count) 
            Assert.AreEqual<int>(1, node.Scope.Count) 
        | "FplMapping", "00b" -> 
            Assert.IsInstanceOfType(parent, typeof<FplFunctionalTerm>) 
        ...
        | "FplMapping", "02c"  
        | "FplMapping", "03c" -> 
            Assert.IsInstanceOfType(parent, typeof<FplMapping>) 
        ...
        | "FplMapping", "00d" -> 
            ...
            Assert.AreEqual<int>(3, node.Scope.Count) // 3 variables
        | "FplMapping", "01a" -> 
            Assert.IsInstanceOfType(parent, typeof<FplMandatoryFunctionalTerm>) 
            Assert.AreEqual<int>(1, parent.ArgList.Count) // mapping
            Assert.AreEqual<int>(0, parent.Scope.Count) 
            Assert.IsInstanceOfType(node, typeof<FplMapping>)
            Assert.AreEqual<int>(0, node.ArgList.Count) 
        ...
        | "FplMapping", "01d" -> 
            ...
            Assert.AreEqual<int>(3, node.Scope.Count) // 3 variables
        | "FplMapping", "03a" -> 
            Assert.IsInstanceOfType(parent, typeof<FplExtension>) 
    ...
    member this.TestStructureFplNegation(nodeType, varVal, fplCode, identifier) =
        ...
        | "FplNegation", "00" ->
            ...
            Assert.AreEqual<int>(1, parent.ArgList.Count)
            Assert.AreEqual<int>(0, parent.Scope.Count)
            Assert.IsInstanceOfType(node, typeof<FplNegation>)
            Assert.AreEqual<int>(1, node.ArgList.Count)
            Assert.AreEqual<int>(0, node.Scope.Count)
        | _ -> failwith($"unmatched test {nodeType} {varVal}")
    ...
    member this.TestStructureFplParent(nodeType, varVal, fplCode, identifier) =
        ...
        match nodeType, varVal with
            ...
            | "FplParent", "00_5"
                ...
                Assert.IsTrue(parent.RefersTo.IsSome)
                Assert.IsTrue(Object.ReferenceEquals(node, (parent.RefersTo.Value)))
                Assert.IsInstanceOfType(node, typeof<FplParent>)
                Assert.AreEqual<int>(0, node.ArgList.Count)
                Assert.IsTrue(node.RefersTo.IsSome)
    ...
    member this.TestStructureFplPredicate(nodeType, varVal, fplCode, identifier) =
        ...
        | "FplPredicate", "00" -> 
            ...
            Assert.AreEqual<int>(0, parent.ArgList.Count)
            Assert.AreEqual<int>(1, parent.Scope.Count)
            Assert.IsInstanceOfType(node, typeof<FplPredicate>)
            Assert.AreEqual<int>(0, node.ArgList.Count)
            Assert.AreEqual<int>(0, node.Scope.Count)
            Assert.AreEqual<string>("T()", node.Represent())
        | "FplPredicate", "01" -> 
    ...
    member this.TestStructureFplPredicateList(nodeType, varVal, fplCode, identifier) =
        ...
        | "FplPredicateList", "00" -> 
            ...
            Assert.AreEqual<int>(2, parent.ArgList.Count)
            Assert.AreEqual<int>(0, parent.Scope.Count)
            Assert.IsInstanceOfType(node, typeof<FplPredicateList>)
    ...
    member this.TestStructureFplProof(nodeType, varVal, fplCode, identifier) =
        ...
        | "FplProof", "00" -> 
            ...
            Assert.IsInstanceOfType(node, typeof<FplProof>)
            Assert.AreEqual<int>(0, node.ArgList.Count) 
            Assert.AreEqual<int>(1, node.Scope.Count)
        | "FplProof", "00x" -> 
            Assert.IsInstanceOfType(parent, typeof<FplTheory>)
    ...
    member this.TestStructureFplProposition(nodeType, varVal, fplCode, identifier) =
        ...
        | "FplProposition", "00" -> 
            ...
            Assert.AreEqual<int>(0, parent.ArgList.Count)
            Assert.AreEqual<int>(1, parent.Scope.Count)
            Assert.IsInstanceOfType(node, typeof<FplProposition>)
            Assert.AreEqual<int>(1, node.ArgList.Count)
            Assert.AreEqual<int>(0, node.Scope.Count)
        | _ -> failwith($"unmatched test {nodeType} {varVal}")
    ...
    member this.TestStructureFplReference(nodeType, varVal, fplCode, identifier) =
        ...
        | "FplReference", "00a" ->
            ...
            Assert.AreEqual<int>(1, parent.ArgList.Count)
            Assert.AreEqual<int>(0, parent.Scope.Count)
            Assert.IsInstanceOfType(node, typeof<FplReference>)
            Assert.AreEqual<int>(0, node.ArgList.Count)
            Assert.AreEqual<int>(0, node.Scope.Count)
            Assert.IsInstanceOfType(node.RefersTo.Value, typeof<FplVariable>)
        ...
        | "FplReference", "01b" ->
            ...
            Assert.AreEqual<int>(2, node.ArgList.Count) // 2 arguments
            Assert.AreEqual<int>(0, node.Scope.Count) 
            Assert.IsTrue(node.RefersTo.IsSome) // one referenced element
            Assert.AreEqual<string>("A", node.FplId) // name of the referenced element
            Assert.IsInstanceOfType(node.RefersTo.Value, typeof<FplPredicate>) // type of the referenced element
        | "FplReference", "01c" ->
            Assert.IsInstanceOfType(parent, typeof<FplAxiom>)
            Assert.AreEqual<int>(1, parent.ArgList.Count)
            Assert.AreEqual<int>(2, parent.Scope.Count) // 2 variables
            ...
            Assert.IsTrue(node.RefersTo.IsSome) // one referenced element
            Assert.AreEqual<string>("A", node.FplId) // name of the referenced element
            Assert.IsInstanceOfType(node.RefersTo.Value, typeof<FplFunctionalTerm>) // type of the referenced element
        | "FplReference", "01d" ->
            Assert.IsInstanceOfType(parent, typeof<FplAxiom>)
            Assert.AreEqual<int>(1, parent.ArgList.Count)
            Assert.AreEqual<int>(0, parent.Scope.Count) 
        ...
        | "FplReference", "01e" ->
            ...
            Assert.IsTrue(node.RefersTo.IsSome) // one referenced element
            Assert.AreEqual<string>("A", node.FplId) // name of the referenced element
            Assert.IsInstanceOfType(node.RefersTo.Value, typeof<FplClass>) // type of the referenced element
        | "FplReference", "01f" ->
            Assert.IsInstanceOfType(parent, typeof<FplAxiom>)
            Assert.AreEqual<int>(1, parent.ArgList.Count)
            Assert.AreEqual<int>(0, parent.Scope.Count) 
            ...
            Assert.AreEqual<string>("A", node.FplId) // name of the referenced element
            Assert.IsInstanceOfType(node.RefersTo.Value, typeof<FplRuleOfInference>) // type of the referenced element
        | "FplReference", "01g" ->
            Assert.IsInstanceOfType(parent, typeof<FplAxiom>)
            Assert.AreEqual<int>(1, parent.ArgList.Count)
            Assert.AreEqual<int>(0, parent.Scope.Count) 
            ...
            Assert.IsTrue(node.RefersTo.IsSome) // referenced element
            Assert.AreEqual<string>("A", node.FplId) // name of the referenced element
            Assert.IsInstanceOfType(node.RefersTo.Value, typeof<FplAxiom>) // type of the referenced element
        | "FplReference", "01h" ->
            Assert.IsInstanceOfType(parent, typeof<FplAxiom>)
            Assert.AreEqual<int>(1, parent.ArgList.Count)
            Assert.AreEqual<int>(0, parent.Scope.Count) 
            ...
            Assert.IsTrue(node.RefersTo.IsSome) // referenced element
            Assert.AreEqual<string>("A", node.FplId) // name of the referenced element
            Assert.IsInstanceOfType(node.RefersTo.Value, typeof<FplTheorem>) // type of the referenced element
        | "FplReference", "01i" ->
            Assert.IsInstanceOfType(parent, typeof<FplAxiom>)
            Assert.AreEqual<int>(1, parent.ArgList.Count)
            Assert.AreEqual<int>(0, parent.Scope.Count) 
            ...
            Assert.IsTrue(node.RefersTo.IsSome) // referenced element
            Assert.AreEqual<string>("A", node.FplId) // name of the referenced element
            Assert.IsInstanceOfType(node.RefersTo.Value, typeof<FplLemma>) // type of the referenced element
        | "FplReference", "01j" ->
            Assert.IsInstanceOfType(parent, typeof<FplAxiom>)
            Assert.AreEqual<int>(1, parent.ArgList.Count)
            Assert.AreEqual<int>(0, parent.Scope.Count) 
            ...
            Assert.IsTrue(node.RefersTo.IsSome) // referenced element
            Assert.AreEqual<string>("A", node.FplId) // name of the referenced element
            Assert.IsInstanceOfType(node.RefersTo.Value, typeof<FplProposition>) // type of the referenced element
        | "FplReference", "01k" ->
            Assert.IsInstanceOfType(parent, typeof<FplAxiom>)
            Assert.AreEqual<int>(1, parent.ArgList.Count)
            Assert.AreEqual<int>(0, parent.Scope.Count) 
            ...
            Assert.IsTrue(node.RefersTo.IsSome) // referenced element
            Assert.AreEqual<string>("A", node.FplId) // name of the referenced element
            Assert.IsInstanceOfType(node.RefersTo.Value, typeof<FplConjecture>) // type of the referenced element
        | "FplReference", "01l" ->
            Assert.IsInstanceOfType(parent, typeof<FplAxiom>)
            Assert.AreEqual<int>(1, parent.ArgList.Count)
            Assert.AreEqual<int>(0, parent.Scope.Count) 
        ...
        | "FplReference", "01m" ->
            ...
            Assert.IsTrue(node.RefersTo.IsSome) // referenced element
            Assert.AreEqual<string>("A$1", node.FplId) // name of the referenced element
            Assert.IsInstanceOfType(node.RefersTo.Value, typeof<FplProof>)         
        | "FplReference", "01n" ->
            Assert.IsInstanceOfType(parent, typeof<FplAxiom>)
            Assert.AreEqual<int>(1, parent.ArgList.Count)
            Assert.AreEqual<int>(0, parent.Scope.Count) 
            ...
            Assert.IsTrue(node.RefersTo.IsSome) // referenced element
            Assert.AreEqual<string>("A", node.FplId) // name of the referenced element
            Assert.IsInstanceOfType(node.RefersTo.Value, typeof<FplExtension>) // type of the referenced element
        | "FplReference", "01o" ->
            Assert.IsInstanceOfType(parent, typeof<FplLocalization>)
            Assert.AreEqual<int>(1, parent.ArgList.Count)
            Assert.AreEqual<int>(1, parent.Scope.Count) 
    ...
    member this.TestStructureFplReturn(nodeType, varVal, fplCode, identifier) =
        ...
        | "FplReturn", "00" ->
            ...
            Assert.IsInstanceOfType(node, typeof<FplReturn>)
    ...
    member this.TestStructureFplRuleOfInference(nodeType, varVal, fplCode, identifier) =
        ...
        | "FplRuleOfInference", "00" -> 
            ...
            Assert.AreEqual<int>(0, parent.ArgList.Count)
            Assert.AreEqual<int>(1, parent.Scope.Count)
            Assert.IsInstanceOfType(node, typeof<FplRuleOfInference>)
            Assert.AreEqual<int>(2, node.ArgList.Count)
            Assert.AreEqual<int>(0, node.Scope.Count)
    ...
    member this.TestStructureFplSelf(nodeType, varVal, fplCode, identifier) =
        ...
        | "FplSelf", "00_2" 
            ...
            Assert.IsTrue(parent.RefersTo.IsSome)
            Assert.IsTrue(Object.ReferenceEquals(node, (parent.RefersTo.Value)))
            Assert.IsInstanceOfType(node, typeof<FplSelf>)
            Assert.AreEqual<int>(0, node.ArgList.Count)
            Assert.IsTrue(node.RefersTo.IsSome)
        // nodes that can be referred to as 'self'
        | "FplSelf", "00" 
    ...
    member this.TestStructureFplTheorem(nodeType, varVal, fplCode, identifier) =
        ...
        | "FplTheorem", "00" -> 
            ...
            Assert.AreEqual<int>(0, parent.ArgList.Count)
            Assert.AreEqual<int>(1, parent.Scope.Count)
            Assert.IsInstanceOfType(node, typeof<FplTheorem>)
            Assert.AreEqual<int>(1, node.ArgList.Count)
            Assert.AreEqual<int>(0, node.Scope.Count)
    ...
    member this.TestStructureFplTheory(nodeType, varVal, fplCode, identifier) =
        ...
        match nodeType, varVal with
        | "FplTheory", "00" ->
            Assert.IsInstanceOfType(parent, typeof<FplRoot>)
            Assert.AreEqual<int>(0, parent.ArgList.Count)
            Assert.AreEqual<int>(1, parent.Scope.Count)
            Assert.IsInstanceOfType(node, typeof<FplTheory>)
            Assert.AreEqual<int>(0, node.ArgList.Count)
            Assert.AreEqual<int>(0, node.Scope.Count)
    ...
    member this.TestStructureFplTranslation(nodeType, varVal, fplCode, identifier) =
        ...
        match nodeType, varVal with
        | "FplTranslation", "01" ->
            Assert.IsInstanceOfType(parent, typeof<FplLanguage>) 
            Assert.AreEqual<int>(3, parent.ArgList.Count)
            Assert.AreEqual<int>(0, parent.Scope.Count)
            Assert.IsInstanceOfType(node, typeof<FplTranslation>)
            Assert.AreEqual<int>(0, node.ArgList.Count)
            Assert.AreEqual<int>(0, node.Scope.Count)
        | "FplTranslation", "02" ->
            Assert.IsInstanceOfType(parent, typeof<FplLanguage>) 
    ...
    member this.TestStructureFplVariable(nodeType, varVal, fplCode, identifier) =
        ...
        | "FplVariable", "00a" ->
            ...
            Assert.AreEqual<int>(1, parent.ArgList.Count)
            Assert.AreEqual<int>(1, parent.Scope.Count)
            Assert.IsInstanceOfType(node, typeof<FplVariable>)
            Assert.AreEqual<int>(0, node.ArgList.Count)
            Assert.AreEqual<int>(0, node.Scope.Count)
        ...
        | "FplVariable", "01b" ->
            ...
            Assert.IsTrue(x.IsSignatureVariable)
        | "FplVariable", "01c" ->
            Assert.IsInstanceOfType(parent, typeof<FplMandatoryPredicate>)
            Assert.AreEqual<int>(1, parent.ArgList.Count)
            Assert.AreEqual<int>(1, parent.Scope.Count)
            Assert.IsInstanceOfType(node, typeof<FplVariable>)
            Assert.AreEqual<int>(0, node.ArgList.Count)
        ...
        // variable to variable
        | "FplVariable", "02a" ->
            Assert.IsInstanceOfType(parent, typeof<FplVariable>)
            Assert.AreEqual<int>(0, parent.ArgList.Count)
            Assert.AreEqual<int>(1, parent.Scope.Count)
        ...
        // variable to variable
        | "FplVariable", "02c" ->
            Assert.IsInstanceOfType(parent, typeof<FplVariableArray>)
            Assert.AreEqual<int>(0, parent.ArgList.Count)
            Assert.AreEqual<int>(1, parent.Scope.Count)
            let p = (parent:?>FplGenericVariable)
            Assert.IsFalse(p.IsInitialized)
        ...
        // translations 
        | "FplVariable", "05a" ->
            Assert.IsInstanceOfType(parent, typeof<FplTranslation>)
            Assert.AreEqual<int>(0, parent.ArgList.Count) 
    ...
    member this.TestStructureFplVariableArray(nodeType, varVal, fplCode, identifier) =
        ...
        | "FplVariableArray", "00" ->
            ...
            Assert.AreEqual<int>(1, parent.ArgList.Count)
            Assert.AreEqual<int>(1, parent.Scope.Count)
            Assert.IsInstanceOfType(node, typeof<FplVariableArray>)
            Assert.AreEqual<int>(0, node.ArgList.Count)
            Assert.AreEqual<int>(0, node.Scope.Count)
