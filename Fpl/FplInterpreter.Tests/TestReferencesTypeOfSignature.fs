...
type TestReferencesTypeOfSignature() =
    ...
    //todo[<DataRow("base11v1", "def pred T() { dec ~v:obj; v };")>]
    //todo[<DataRow("base11v2", "def pred T() { dec ~v:ind; v };")>]
    //todo[<DataRow("base11v3", "def pred T() { dec ~v:Nat; v };")>]
    //[<DataRow("base12", "self")>]
    //[<DataRow("base13", "1")>]
    //[<DataRow("base11a", "v.x")>]
    ...
    //[<DataRow("base13b", "1()")>]
    //todo[<DataRow("base10c", "def pred Test(x,y:Nat) {true} def pred T() { dec ~x,y:Nat; Test(x, y) };")>]
