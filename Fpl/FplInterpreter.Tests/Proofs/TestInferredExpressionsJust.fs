...
type TestInferredExpressionsJust() =
    ...
    [<DataRow("74b", """def cl N def cl M def cl K ax ax A1 {dec p,q,r:pred; (p ⇔ q) ⩡ r ∨ ¬p} thm T { (∀ x:obj {x is N} ⇔ ∃ y:obj {y is M}) ⩡ (true ∨ false) ∧ ¬∀ z:obj {z is N} } prf T$1 { 1. byax A1 |- (∀ x:obj {x is N} ⇔ ∃ y:obj {y is M}) ⩡ (true ∨ false) ∧ ¬∀ z:obj {z is N} }""", "(∀ x:obj {x is N} ⇔ ∃ y:obj {y is M}) ⩡ ((true ∨ false) ∧ ¬∀ z:obj {z is N})", 1)>]
