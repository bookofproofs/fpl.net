...
type TestInferredExpressionsJust() =
    ...
    [<DataRow("74", """def cl N def cl M def cl K ax A1 {dec p,q,r:pred; xor(iif(p, q), and(r, not p))} thm T {xor(iif(all x:obj {is(x,N)}, ex y:obj {is(y,M)}), and(or(true, false), not all z:obj {is(z,K)}))} prf T$1 { 1. byax A1 |- xor(iif(all x:obj {is(x,N)}, ex y:obj {is(y,M)}), and(or(true, false), not all z:obj {is(z,K)})) }""", "((∀ x:obj {x is N} ⇔ ∃ y:obj {y is M}) ⩡ ((true ∨ false) ∧ ¬∀ z:obj {z is K}))", 1)>]
    ...
    [<DataRow("60", """def cl N def cl M def cl K ax A1 {dec p,q:pred; not iif(p, q)} thm T {not iif(ex x:obj {is(x,N)}, and(true, false))} prf T$1 { 1. byax A1 |- not iif(ex x:obj {is(x,N)}, and(true, false)) }""", "¬(∃ x:obj {x is N} ⇔ (true ∧ false))", 1)>]
