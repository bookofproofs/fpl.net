...
type TestFplValueScopeParent() =
    ...
    member this.TestProperties(var) =
        ...
        | Some (r:FplRoot,theory:FplValue,block:FplValue,t1:FplValue,t3:FplValue,t5:FplValue,t7:FplValue,t9:FplValue,t11:FplValue,t13:FplValue) -> 
            ...
            | "t1" -> Assert.AreEqual<FplValue>(block, t1.Parent.Value)
            | "t3" -> Assert.AreEqual<FplValue>(block, t3.Parent.Value)
            | "t5" -> Assert.AreEqual<FplValue>(block, t5.Parent.Value)
            | "t7" -> Assert.AreEqual<FplValue>(block, t7.Parent.Value)
            | "t9" -> Assert.AreEqual<FplValue>(block, t9.Parent.Value)
            | "t11" -> Assert.AreEqual<FplValue>(block, t11.Parent.Value)
    ...
    member this.TestLocalizationParent(var, predName, predDecl, trslCode) =
        ...
        | Some st -> 
            ...
            match var with
            | "base0" -> Assert.AreEqual<FplValue>(theory, pred.Parent.Value)
