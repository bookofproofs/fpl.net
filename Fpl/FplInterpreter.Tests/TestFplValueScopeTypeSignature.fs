...
type TestFplValueScopeTypeSignature() =
    ...
    member this.TestVariablesInBlock(var) =
        ...
        | Some (r,theory,block,x,y,xw,xu,xv,yw,yu,yv,xwa,xwb,xwc,xua,xub,xuc,xva,xvb,xvc,ywa,ywb,ywc,yua,yub,yuc,yva,yvb,yvc) ->
            ...
            | "x" -> Assert.AreEqual(["x"], x.TypeSignature)
            | "y" -> Assert.AreEqual(["y"], y.TypeSignature)
            | "xu" -> Assert.AreEqual(["u"], xu.TypeSignature)
            | "xv" -> Assert.AreEqual(["v"], xv.TypeSignature)
            | "xw" -> Assert.AreEqual(["w"], xw.TypeSignature)
            | "yu" -> Assert.AreEqual(["u"], yu.TypeSignature)
            | "yv" -> Assert.AreEqual(["v"], yv.TypeSignature)
            | "yw" -> Assert.AreEqual(["w"], yw.TypeSignature)
            | "xua" -> Assert.AreEqual(["a"], xua.TypeSignature)
            | "xub" -> Assert.AreEqual(["b"], xub.TypeSignature)
            | "xuc" -> Assert.AreEqual(["c"], xuc.TypeSignature)
            | "xva" -> Assert.AreEqual(["a"], xva.TypeSignature)
            | "xvb" -> Assert.AreEqual(["b"], xvb.TypeSignature)
            | "xvc" -> Assert.AreEqual(["c"], xvc.TypeSignature)
            | "xwa" -> Assert.AreEqual(["a"], xwa.TypeSignature)
            | "xwb" -> Assert.AreEqual(["b"], xwb.TypeSignature)
            | "xwc" -> Assert.AreEqual(["c"], xwc.TypeSignature)
            | "yua" -> Assert.AreEqual(["a"], yua.TypeSignature)
            | "yub" -> Assert.AreEqual(["b"], yub.TypeSignature)
            | "yuc" -> Assert.AreEqual(["c"], yuc.TypeSignature)
            | "yva" -> Assert.AreEqual(["a"], yva.TypeSignature)
            | "yvb" -> Assert.AreEqual(["b"], yvb.TypeSignature)
            | "yvc" -> Assert.AreEqual(["c"], yvc.TypeSignature)
            | "ywa" -> Assert.AreEqual(["a"], ywa.TypeSignature)
            | "ywb" -> Assert.AreEqual(["b"], ywb.TypeSignature)
            | "ywc" -> Assert.AreEqual(["c"], ywc.TypeSignature)
            | _ -> Assert.IsTrue(false)
        | None -> 
            Assert.IsTrue(false)
