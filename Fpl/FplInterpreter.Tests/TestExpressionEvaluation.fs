...
type TestExpressionEvaluation() =
    ...
    [<DataRow("VP1", "def pred Test() {is(true, pred())};", LiteralTrue)>] // true is pred()
    [<DataRow("VP1a", "def pred Test() {is(true, pred)};", LiteralTrue)>] // true is pred
    [<DataRow("VP2", "def pred Test() {is(false, pred())};", LiteralTrue)>] // false is pred()
    [<DataRow("VP2a", "def pred Test() {is(false, pred)};", LiteralTrue)>] // false is pred
    [<DataRow("VP3", "def pred Test() {dec ~x:pred(); is(x, pred())};", LiteralTrue)>] // x typed pred() is pred()
    [<DataRow("VP3a", "def pred Test() {dec ~x:pred(); is(x, pred)};", LiteralFalse)>] // x typed pred() is not pred
    [<DataRow("VP3b", "def pred Test() {dec ~x:pred; is(x, pred)};", LiteralTrue)>] // x typed pred is not pred
    [<DataRow("VP3c", "def pred Test() {dec ~x:pred; is(x, pred())};", LiteralFalse)>] // x typed pred is not pred()

    // match with and
    [<DataRow("VP4a", "def pred Test() {dec ~x,y:pred(); is( and(x,y), pred()) };", LiteralTrue)>] // and is pred()
    [<DataRow("VP4b", "def pred Test() {dec ~x:pred(),y:pred; is( and(x,y), pred()) };", LiteralTrue)>] // and is pred()
    [<DataRow("VP4c", "def pred Test() {dec ~x:pred,y:pred(); is( and(x,y), pred()) };", LiteralTrue)>] // and is pred()
    [<DataRow("VP4c", "def pred Test() {dec ~x:pred,y:pred; is( and(x,y), pred()) };", LiteralTrue)>] // and is pred()
    [<DataRow("VP5a", "def pred Test() {dec ~x,y:pred(); is( and(x,y), pred) };", LiteralTrue)>] // and is pred
    [<DataRow("VP5b", "def pred Test() {dec ~x:pred(),y:pred; is( and(x,y), pred) };", LiteralTrue)>] // and is pred
    [<DataRow("VP5c", "def pred Test() {dec ~x:pred,y:pred(); is( and(x,y), pred) };", LiteralTrue)>] // and is pred
    [<DataRow("VP5d", "def pred Test() {dec ~x:pred,y:pred; is( and(x,y), pred) };", LiteralTrue)>] // and is pred
    [<DataRow("VP6a", "def pred Test() {dec ~x,y:pred(); is( and(x,y), pred()) };", LiteralFalse)>] // and is not pred()
    [<DataRow("VP6b", "def pred Test() {dec ~x:pred(),y:pred; is( and(x,y), pred()) };", LiteralFalse)>] // and is not pred()
    [<DataRow("VP6c", "def pred Test() {dec ~x:pred,y:pred(); is( and(x,y), pred()) };", LiteralFalse)>] // and is not pred()
    ...
    [<DataRow("VP7b", "def pred Test() {dec ~x:pred,y:pred; is( and(x,y), pred(pred,pred())) };", LiteralFalse)>] // and(pred,pred) is not pred(pred,pred())
    [<DataRow("VP7c", "def pred Test() {dec ~x:pred,y:pred; is( and(x,y), pred(pred(),pred)) };", LiteralFalse)>] // and(pred,pred) is not pred(pred(),pred) 
    [<DataRow("VP7d", "def pred Test() {dec ~x:pred,y:pred; is( and(x,y), pred(pred(),pred())) };", LiteralFalse)>] // and(pred,pred) is not pred(pred(),pred())
    [<DataRow("VP7e", "def pred Test() {dec ~x:pred,y:pred(); is( and(x,y), pred(pred,pred)) };", LiteralFalse)>] // and(pred,pred()) is not pred(pred,pred)
    [<DataRow("VP7f", "def pred Test() {dec ~x:pred,y:pred(); is( and(x,y), pred(pred,pred())) };", LiteralTrue)>] // and(pred,pred()) is pred(pred,pred())
    [<DataRow("VP7g", "def pred Test() {dec ~x:pred,y:pred(); is( and(x,y), pred(pred(),pred)) };", LiteralFalse)>] // and(pred,pred()) is not pred(pred(),pred)
    [<DataRow("VP7h", "def pred Test() {dec ~x:pred,y:pred(); is( and(x,y), pred(pred(),pred())) };", LiteralFalse)>] // and(pred,pred()) is not pred(pred(),pred())
    [<DataRow("VP7i", "def pred Test() {dec ~x:pred(),y:pred; is( and(x,y), pred(pred,pred)) };", LiteralFalse)>] // and(pred(),pred) is not pred(pred,pred)
    [<DataRow("VP7j", "def pred Test() {dec ~x:pred(),y:pred; is( and(x,y), pred(pred,pred())) };", LiteralFalse)>] // and(pred(),pred) is not pred(pred,pred())
    [<DataRow("VP7k", "def pred Test() {dec ~x:pred(),y:pred; is( and(x,y), pred(pred(),pred)) };", LiteralTrue)>] // and(pred(),pred) is pred(pred(),pred)
    [<DataRow("VP7l", "def pred Test() {dec ~x:pred(),y:pred; is( and(x,y), pred(pred(),pred())) };", LiteralFalse)>] // and(pred(),pred) is not pred(pred(),pred())
    [<DataRow("VP7m", "def pred Test() {dec ~x:pred(),y:pred(); is( and(x,y), pred(pred,pred)) };", LiteralFalse)>] // and(pred(),pred()) is not pred(pred,pred)
    [<DataRow("VP7n", "def pred Test() {dec ~x:pred(),y:pred(); is( and(x,y), pred(pred,pred())) };", LiteralFalse)>] // and(pred(),pred()) is not pred(pred,pred())
    [<DataRow("VP7o", "def pred Test() {dec ~x:pred(),y:pred(); is( and(x,y), pred(pred(),pred)) };", LiteralFalse)>] // and(pred(),pred()) is not pred(pred(),pred)
    [<DataRow("VP7p", "def pred Test() {dec ~x:pred(),y:pred(); is( and(x,y), pred(pred(),pred())) };", LiteralTrue)>] // and(pred(),pred()) is pred(pred(),pred())

    [<DataRow("ST4b", "def pred Test() {dec ~x,y:pred(); is( or(x,y), pred()) };", LiteralTrue)>]
    [<DataRow("ST4c", "def pred Test() {dec ~x,y:pred(); is( xor(x,y), pred()) };", LiteralTrue)>]
    [<DataRow("ST4d", "def pred Test() {dec ~x,y:pred(); is( impl(x,y), pred()) };", LiteralTrue)>]
    [<DataRow("ST4e", "def pred Test() {dec ~x,y:pred(); is( iif(x,y), pred()) };", LiteralTrue)>]
    [<DataRow("ST4f", "dec pred P(x:obj) def pred Test() { is( all x:obj { P(x) }, pred()) };", LiteralTrue)>]
    [<DataRow("ST4g", "dec pred P(x:obj) def pred Test() { is( ex x:obj { P(x) }, pred()) };", LiteralTrue)>]
    [<DataRow("ST4h", "dec pred P(x:obj) def pred Test() { is( ex$1 x:obj { P(x) }, pred()) };", LiteralTrue)>]

    // mismatch with other simple types 
    [<DataRow("ST4_obj", "def pred Test() {dec ~x:pred(); is(x, obj)};", LiteralFalse)>]
    [<DataRow("ST4_ind", "def pred Test() {dec ~x:pred(); is(x, ind)};", LiteralFalse)>]
    [<DataRow("ST4_pred", "def pred Test() {dec ~x:pred(); is(x, pred)};", LiteralFalse)>]
    [<DataRow("ST4_func", "def pred Test() {dec ~x:pred(); is(x, func)};", LiteralFalse)>]
    ...
    [<DataRow("ST2c_pred()", "def pred Test() {dec ~x:func(y:obj)->func; is(x, pred())};", LiteralFalse)>]
    [<DataRow("ST2d_pred()", "def pred Test() {dec ~x:func(y:obj)->func(z:pred)->pred; is(x, pred())};", LiteralFalse)>]
    [<DataRow("ST3_pred()", "def pred Test() {dec ~x:pred; is(x, pred())};", LiteralFalse)>]
    [<DataRow("ST3a_pred()", "def pred Test() {dec ~x:pred(); is(x, pred())};", LiteralFalse)>]
    [<DataRow("ST3b_pred()", "def pred Test() {dec ~x:pred; is(x, pred())};", LiteralFalse)>]
    [<DataRow("ST3c_pred()", "def pred Test() {dec ~x:pred(y:obj); is(x, pred())};", LiteralFalse)>]
    [<DataRow("ST4_pred()", "def pred Test() {dec ~x:pred(); is(x, func)};", LiteralFalse)>]

    [<DataRow("NP4", "def pred Test() {dec ~x:pred(); is(x, pred())};", LiteralFalse)>]

    [<DataRow("NP_4", "def pred Test() {dec ~x:pred(); is(x, pred(a:obj))};", LiteralFalse)>]

    [<DataRow("NF4", "def pred Test() {dec ~x:pred(); is(x, func()->ind)};", LiteralFalse)>]

    [<DataRow("NF_4", "def pred Test() {dec ~x:pred(); is(x, func(a:obj)->ind)};", LiteralFalse)>]

    // match with the type pred()
    [<DataRow("MS2_", "def pred A(z:obj) def pred Test() {is(A, pred())};", LiteralTrue)>] // true: pred matches signature A(obj)
    [<DataRow("MS2a_", "def pred A(z:obj) def pred Test() {dec ~x:obj; is(A(x), pred())};", LiteralTrue)>] // true: pred matches pred()ue A(obj) 
    [<DataRow("MS2b_", "def pred A(z:obj) def pred Test() {dec ~x:ind; is(A(x), pred())};", LiteralFalse)>] // false: pred does not match pred()ue A(ind) since it does not match A(obj)
    [<DataRow("MS2c_", "def pred A(z:ind) def pred Test() {dec ~x:ind; is(A(x), pred())};", LiteralTrue)>] // true: pred matches pred()ue A(ind) 
    [<DataRow("MS2d_", "def pred A(z:ind) def pred Test() {is(A, pred())};", LiteralTrue)>] // true: pred matches signature A(ind)
    [<DataRow("MS2e_", "ax A {true} def pred Test() {is(A, pred())};", LiteralTrue)>] // true: pred matches signature A (axiom)
    [<DataRow("MS2f_", "thm A {true} def pred Test() {is(A, pred())};", LiteralTrue)>] // true: pred matches signature A (theorem)
    [<DataRow("MS2g_", "lem A {true} def pred Test() {is(A, pred())};", LiteralTrue)>] // true: pred matches signature A (lemma)
    [<DataRow("MS2h_", "prop A {true} def pred Test() {is(A, pred())};", LiteralTrue)>] // true: pred matches signature A (proposition)
    [<DataRow("MS2i_", "conj A {true} def pred Test() {is(A, pred())};", LiteralTrue)>] // true: pred does matches signature A (conjecture)
    [<DataRow("MS2j_", "cor A$1 {true} def pred Test() {is(A$1, pred())};", LiteralTrue)>] // true: pred matches signature A$1 (corollary)
    [<DataRow("MS2k_", "proof A$1 {1. |- trivial} def pred Test() {is(A$1, pred())};", LiteralFalse)>] // false: pred does not match signature A$1 (proof)
    [<DataRow("MS2l_", "inf A {pre:true con:true} def pred Test() {is(A, pred())};", LiteralFalse)>] // false: pred does not match signature A (rule of inference)
    [<DataRow("MS2m_", "def func A()->obj def pred Test() {is(A, pred())};", LiteralFalse)>] // false: pred does not match signature A (functional term)
    [<DataRow("MS2n_", "ext A x@/\d+/ -> obj {dec ~y:obj; return y} def pred Test() {is(A, pred())};", LiteralFalse)>] // false: pred does not match signature A (extension)
    [<DataRow("MS2o_", "def cl A def pred Test() {is(A, pred())};", LiteralFalse)>] // false: pred does not match signature A (class)
