...
type TestExpressionEvaluation() =
    ...
    [<DataRow("def pred T() { and(true,and(x,true)) }", LiteralUndet)>]
    [<DataRow("def pred T() { and(true,and(true,x)) }", LiteralUndet)>]
    [<DataRow("def pred T() { and(x,and(true,true)) }", LiteralUndet)>]
    ...
    [<DataRow("def pred T() { or(false,or(x,false)) }", LiteralUndet)>]
    [<DataRow("def pred T() { or(or(false,false),x) }", LiteralUndet)>]
    [<DataRow("def pred T() { or(x,or(false,false)) }", LiteralUndet)>]
    ...
    [<DataRow("def pred T() { xor(xor(false,x),false) }", LiteralUndet)>]
    [<DataRow("def pred T() { xor(false,xor(false,x)) }", LiteralUndet)>]
    [<DataRow("def pred T() { xor(x,xor(false,false)) }", LiteralUndet)>]
    ...
    [<DataRow("def pred T() { impl(x,false) }", LiteralUndet)>]
    [<DataRow("def pred T() { impl(false,x) }", LiteralUndet)>]
    ...
    [<DataRow("def pred T() { iif(x,false) }", LiteralUndet)>]
    [<DataRow("def pred T() { iif(false,x) }", LiteralUndet)>]
    ...
    [<DataRow("MS2p7", "def cl A {intr prty pred X(x:obj) } def pred Test() {dec a:obj; is(A.X(a), pred)}", LiteralTrue)>] // false: pred matches by value A.X(obj) 
    // ... using properties of instances
    ...
    [<DataRow("MS2q7", "def cl A {intr prty pred X(x:obj) } def pred Test() {dec a:obj o:A o:=A(); is(o.X(a), pred)}", LiteralTrue)>] // true: pred matches by value A.X(obj) 
    // ... using properties of predicates
    ...
    [<DataRow("MS2s7", "def func A()->obj {intr prty pred X(x:obj) } def pred Test() {dec a:obj; is(A.X(a), pred)}", LiteralTrue)>] // true: pred matches by value A.X(obj) 
