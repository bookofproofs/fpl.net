...
type TestInterpreterErrors() =
    ...
    [<DataRow("def object SomeClass:Obj {intrinsic} def object SomeClass:Obj {intrinsic} ;", 1)>]
    [<DataRow("def object SomeClass:Obj {intrinsic} ;", 0)>]
    ...
    [<DataRow("""def obj B:Obj {intr} def obj A:Obj {dec ~x:Obj; ctor A(y:B) {} };""", 0)>]
    ...
    [<DataRow("def obj Test:Obj {intr} proof Test$1 {1. |- trivial};", 1)>]
    ...
    [<DataRow("def obj Test:Obj {intr} corollary Test$1 {true};", 1)>]
    ...
    [<DataRow("""def obj A:Obj {intr} def pred T() {dec ~x:A x:=A; x};""", 1)>]
    [<DataRow("""def obj A:Obj {intr} def pred T() {dec ~x:A x:=A(); x};""", 0)>]
    [<DataRow("uses Fpl.Commons.Structures ;", 0)>]
    [<TestMethod>]
    ...
    [<DataRow("def obj Test:Obj {ctor TestTypo(x:Nat) {}};", 1)>]
    [<DataRow("def obj Test:Obj {ctor TestTypo1() {}};", 1)>]
    [<DataRow("def obj Test:Obj {ctor Test() {}};", 0)>]
    [<DataRow("def obj Test:Obj {dec ~x:Obj x := 0; ctor Test() {dec base.obj(); }};", 0)>]
    [<DataRow("uses Fpl.Commons.Structures ;", 0)>]
    [<TestMethod>]
    ...
    [<DataRow("def obj Test:Obj {intr};", 0)>]
    [<DataRow("def obj Test:Test {intr};", 1)>]
    [<DataRow("def obj Test:Test1, Test2, Test3 {intr};", 0)>]
    [<DataRow("def obj Test:Test1, Test2, Test3, Test {intr};", 1)>]
    [<DataRow("uses Fpl.Commons.Structures ;", 0)>]
    [<TestMethod>]
    ...
    [<DataRow("def obj Test:Obj {intr};", 0)>]
    [<DataRow("def obj Test:Set {intr};", 1)>]
    [<DataRow("def object Set: Obj {intr} def obj Test:Set {intr};", 0)>]
    [<DataRow("def obj Set:Obj {intr} def obj Test:Set {intr};", 0)>]
    [<DataRow("def obj Set:Obj {intr} def obj Test:SetTypo {intr};", 1)>]
    [<DataRow("def obj Set:Obj {intr} def pred Test() {dec ~x:Set; true};", 0)>]
    [<DataRow("def obj Set:Obj {intr} def pred Test() {dec ~x:Object; is(x,Set)};", 0)>]

    [<DataRow("def obj A:Obj {intr} thm T {true} proof T$1 {1. bydef A |- trivial };", 0)>]
    [<DataRow("def obj A:Obj {intr} thm T {true} proof T$1 {1. bydef B |- trivial };", 1)>]
    ...
    [<DataRow("def object Set: Obj {intr} def pred IsEmpty(x: Set) {true};", 0)>]
    [<DataRow("uses Fpl.Commons.Structures ;", 0)>]
    [<TestMethod>]
    ...
    [<DataRow("def obj A:Obj {intr};", 0)>]
    [<DataRow("def obj A:Obj {intr} def obj B:A,obj {intr};", 1)>]
    [<DataRow("def obj A:Obj {intr} def obj B:A {intr} def obj C:B,A {intr};", 1)>]
    [<DataRow("uses Fpl.SetTheory def obj Test:EmptySet,Set {intr};", 1)>]
    [<DataRow("uses Fpl.SetTheory def obj Test:Set, EmptySet {intr};", 1)>]
    [<DataRow("def obj A:Obj {intr} def obj B:A {intr} def obj C:A,B {intr};", 1)>]
    [<DataRow("def obj A:Obj {intr} def obj B:A {intr} def obj C:B {intr};", 0)>]
    [<DataRow("uses Fpl.SetTheory def obj Test:EmptySet {intr};", 0)>]
    [<DataRow("uses Fpl.SetTheory def obj Test:Set {intr};", 0)>]
    [<DataRow("uses Fpl.Commons uses Fpl.SetTheory def obj Test:Set {intr};", 0)>]
    [<DataRow("def obj A:Obj {intr} def obj B:A {intr} def obj C:A {intr};", 0)>]
    [<DataRow("def obj A:Obj {intr} def obj B:A {intr} def obj C:A,A {intr};", 1)>]
    [<DataRow("def obj A:Obj {intr} def obj B:A {intr} def obj C:Obj,object {intr};", 1)>]
    [<DataRow("def obj A:Obj {intr} def obj B:A {intr} def obj C:Object,D,E,obj {intr};", 1)>]
    [<DataRow("uses Fpl.SetTheory def obj Test:Set,obj {intr};", 1)>]
    [<DataRow("uses Fpl.SetTheory def obj Test:EmptySet,obj {intr};", 1)>]
    [<DataRow("uses Fpl.SetTheory def obj Test:Obj,Set {intr};", 1)>]
    [<DataRow("uses Fpl.SetTheory def obj Test:Obj,EmptySet {intr};", 1)>]
    [<DataRow("uses Fpl.Commons.Structures ;", 0)>]
    [<TestMethod>]
    ...
    [<DataRow("def obj A:Obj {intr} def obj B:Obj {intr} def obj C:Obj {intr} def obj D:A,B,C,E {ctor D() {dec base.A() base.B() base.C() base.F(); } };", 1)>]
    [<DataRow("def obj A:Obj {intr} def obj B:A {ctor B() {dec base.A(); } };", 0)>]
    [<DataRow("def obj A:Obj {intr} def obj B:A {ctor B() {dec base.C(); } };", 1)>]
    [<DataRow("def obj A:Obj { ctor A() {dec base.obj(); } };", 0)>]
    [<DataRow("def obj A:Obj { ctor A() {dec base.B(); } };", 1)>]
    [<DataRow("def obj A:C { ctor A() {dec base.obj(); } };", 1)>]
    [<DataRow("uses Fpl.SetTheory def obj Test:Set {ctor Test() {dec base.obj(); } };", 1)>]
    [<DataRow("uses Fpl.SetTheory def obj Test:Set {ctor Test() {dec base.Set(); } };", 0)>]
    [<DataRow("uses Fpl.Commons.Structures ;", 0)>]
    [<TestMethod>]
    ...
    [<DataRow("06a", "def obj Nat: Obj {intr} ax T {all x,y:Nat {del.Equal(x,y)}};", 0, "missing error message")>]
    ...
    [<DataRow("07a", "def obj Nat: Obj {intr} ax T {exn$1 x:Nat {del.Equal(x,@1)}};", 0, "missing error message")>]
    [<DataRow("07b", "ax T {exn$1 x:Obj {del.Equal(x,$1)}};", 0, "missing error message")>]
    [<DataRow("07b_", "def obj Nat: Obj {intr} ax T {exn$1 x:Nat {del.Equal(x,$1)}};", 0, "missing error message")>]
    [<DataRow("08", """ax T {all n:Obj {exn$1 y:Obj {del.Equal(y,n)}}};""", 0, "missing error message")>]
    [<DataRow("08a", """def obj Nat: Obj {intr} ax T {all n:Nat {exn$1 y:Nat {del.Equal(y,n)}}};""", 0, "missing error message")>]
    ...
    [<DataRow("00a", """def obj A:Obj {dec ~x:Obj x:=parent; ctor A() {}};""", 1)>]
    [<DataRow("00b", """def obj A:Obj {ctor A() {dec ~x:Obj x:=parent;}};""", 0)>]
    [<DataRow("00c", """def obj A:Obj {intr property pred T() { is(parent,A) } };""", 0)>]
    [<DataRow("00d", """def obj A:Obj {intr property func T()->obj {dec ~x:Obj x:=parent; return x } };""", 0)>]
    [<DataRow("00e", """def obj A:Obj {intr property pred T() { is(parent,A) } };""", 0)>]
    [<DataRow("00f", """def obj A:Obj {intr property func T()->obj {dec ~x:Obj x:=parent; return x } };""", 0)>]
    ...
    [<DataRow("00a", """def obj A:Obj {dec ~x:Obj x:=self; ctor A() {}};""", 0)>]
    [<DataRow("00b", """def obj A:Obj {ctor A() {dec ~x:Obj x:=self;}};""", 1)>]
    [<DataRow("00c", """def obj A:Obj {intr property pred T() { is(self,A) } };""", 0)>]
    [<DataRow("00d", """def obj A:Obj {intr property func T()->obj {dec ~x:Obj x:=self; return x } };""", 0)>]
    [<DataRow("00e", """def obj A:Obj {intr property pred T() { is(self,A) } };""", 0)>]
    [<DataRow("00f", """def obj A:Obj {intr property func T()->obj {dec ~x:Obj x:=self; return x } };""", 0)>]
    ...
    [<DataRow("00", "def obj A:Obj {intr} def obj B:Obj {intr} def obj C:Obj {intr} def obj D:A,B,C {ctor D() {dec base.A(); } };", 2)>]
    [<DataRow("00a", "def obj A:Obj {intr} def obj B:Obj {intr} def obj C:Obj {intr} def obj D:A,B,C {ctor D() {dec base.B(); } };", 2)>]
    [<DataRow("00b", "def obj A:Obj {intr} def obj B:Obj {intr} def obj C:Obj {intr} def obj D:A,B,C {ctor D() {dec base.C(); } };", 2)>]
    [<DataRow("00c", "def obj A:Obj {intr} def obj B:Obj {intr} def obj C:Obj {intr} def obj D:A,B,C {ctor D() {dec base.A() base.B(); } };", 1)>]
    [<DataRow("00d", "def obj A:Obj {intr} def obj B:Obj {intr} def obj C:Obj {intr} def obj D:A,B,C {ctor D() {dec base.A() base.C(); } };", 1)>]
    [<DataRow("00e", "def obj A:Obj {intr} def obj B:Obj {intr} def obj C:Obj {intr} def obj D:A,B,C {ctor D() {dec base.B() base.C(); } };", 1)>]
    [<DataRow("00f", "def obj A:Obj {intr} def obj B:Obj {intr} def obj C:Obj {intr} def obj D:A,B,C {ctor D() {dec base.A() base.B() base.C(); } };", 0)>]
    [<DataRow("01", "def obj A:Obj {intr} def obj B:A {ctor B() {dec base.A(); } };", 0)>]
    [<DataRow("01a", "def obj A:Obj {intr} def obj B:A {ctor B() {} };", 1)>]
    [<DataRow("02", "def obj A:Obj { ctor A() {dec base.obj(); } };", 0)>]
    [<DataRow("02a", "def obj A:Obj { ctor A() {} };", 1)>]
    [<DataRow("03", "def obj A:Obj { ctor A() {dec base.obj(); } };", 0)>]
    [<DataRow("03a", "def obj A:C { ctor A() {dec base.obj(); } };", 1)>]
    [<DataRow("03b", "def obj A:Obj {intr} def obj B:A {intr} def obj C:B { ctor C() {dec base.obj(); } };", 1)>]
    [<DataRow("04", "uses Fpl.SetTheory def obj Test:Set {ctor Test() {dec base.obj(); } };", 1)>]
    [<DataRow("04a", "uses Fpl.SetTheory def obj Test:Set {ctor Test() {dec base.Set(); } };", 0)>]
    [<DataRow("99", "uses Fpl.Commons.Structures ;", 0)>]
    [<TestMethod>]
    ...
    [<DataRow("01", "def obj A:Obj {intr} def obj B:A {ctor B() {dec base.A(); } };", 0)>]
    [<DataRow("01a", "def obj A:Obj {intr} def obj B:A {ctor B() {dec base.A() base.A(); } };", 1)>]
    [<DataRow("02", "def obj A:Obj { ctor A() {dec base.obj(); } };", 0)>]
    [<DataRow("02a", "def obj A:Obj { ctor A() {dec base.obj() base.obj(); } };", 1)>]
    [<DataRow("99", "uses Fpl.Commons.Structures ;", 0)>]
    [<TestMethod>]
    ...
    [<DataRow("00a", "def obj A:Obj {intr} thm T {true} proof T$1 {1. bydef A |- trivial };", 0)>]
    [<DataRow("00b", "def obj A:Obj {intr} def obj B:A {intr} thm T {true} proof T$1 {1. bydef A |- trivial };", 0)>]
    ...
    [<DataRow("01e", """def obj A:Obj {intr} ax T { A };""", 1)>]
    ...
    [<DataRow("""axiom A {impl(T,true)};""", "Cannot evaluate `implication`; expecting a predicate argument `T`, got `undef`.")>]
    [<DataRow("""def obj T:Obj {intr} axiom A {impl(T,true)};""", "Cannot evaluate `implication`; expecting a predicate argument `T`, got `undef`.")>]
    [<TestMethod>]
    ...
    [<DataRow("00", "def obj A:Obj {intr} thm T {true} proof T$1 {1. bydef A |- trivial };", 0)>]
    [<DataRow("00a", "def obj A:Obj {intr} thm T {true} proof T$1 {1. A |- trivial };", 1)>]
    [<DataRow("00b", "def obj A:Obj {intr} thm T {true} proof T$1 {1. byax A |- trivial };", 1)>]
    [<DataRow("00c", "def obj A:Obj {intr} thm T {true} proof T$1 {1. byconj A |- trivial };", 1)>]
    [<DataRow("00d", "def obj A:Obj {intr} thm T {true} proof T$1 {1. byinf A |- trivial };", 1)>]
    ...
    [<DataRow("03l", "def obj A:Obj {intr} thm T {true} proof T$1 {1. byax A |- trivial };", 1)>]
    ...
    [<DataRow("04l", "def obj A:Obj {intr} thm T {true} proof T$1 {1. byconj A |- trivial };", 1)>]
    ...
    [<DataRow("05f", "def obj A:Obj {intr} thm T {true} proof T$1 {1. A |- trivial };", 1)>]
    ...
    [<DataRow("08k", "def obj A:Obj {intr} thm T {true} proof T$1 {1. byinf A |- trivial };", 1)>]
    ...
    [<DataRow("09j", "def obj A:Obj {intr} thm T {true} proof T$1 {1. A$1:1 |- trivial };", 1)>]
    ...
    [<DataRow("10j", "def obj A:Obj {intr} thm T {true} proof T$1 {1. bycor A$1 |- trivial };", 1)>]
    ...
    [<DataRow("0a", "def obj A:Obj {intr} thm T {true} proof T$1 {1. A$1:1 |- trivial };", 1)>]
    ...
    [<DataRow("3a", "def obj A:Obj {intr} thm T {true} proof T$1 {1. A$1 |- trivial };", 1)>]
    ...
    [<DataRow("z0a", "def obj A:Obj {intr} thm T {true} proof T$1 {1. bydef A |- trivial };", 0)>]
    ...
    [<DataRow("23", """def obj A:Obj symbol "0" {intr} axiom T {0} ;""", 0)>]
    [<DataRow("24", """def obj A:Obj symbol "1" {intr} axiom T {0} ;""", 1)>]
    [<DataRow("25", """def obj A:Obj {intr} axiom T {0} ;""", 1)>]
    [<DataRow("99", "uses Fpl.Commons.Structures ;", 0)>]
    [<TestMethod>]
    ...
    [<DataRow("02", "def obj Nat:Obj {intr} def func Test()->obj {dec ~x:Nat; return x};", 0)>]
    [<DataRow("03", "def obj Nat:Obj {intr} def func Test()->Nat {dec ~x:Nat; return x};", 0)>] // Nat is obj, no error
    [<DataRow("03a", "def obj A:Obj {intr} def obj B:A {intr} def func Test()->B {dec ~x:B; return x};", 0)>] // x is B, no error
    [<DataRow("03b", "def obj A:Obj {intr} def obj B:A {intr} def func Test()->A {dec ~x:B; return x};", 0)>] // x is also A, no error
    [<DataRow("03c", "def obj A:Obj {intr} def obj B:A {intr} def func Test()->obj {dec ~x:B; return x};", 0)>] // x is also obj, no error
    [<DataRow("04", "def obj Nat:Obj {intr} def func Test()->obj {dec ~x:Obj; return x};", 0)>]
    [<DataRow("04a", "def obj Nat:Obj {intr} def func Test()->Nat {dec ~x:Obj; return x};", 1)>] // obj is not Nat, error
    ...
    [<DataRow("00", "def obj Test:Obj {intr};", 0)>]
    [<DataRow("01", "def obj Test:Set {intr};", 0)>] // this should cause the ID010 error only and not SIG04
    [<DataRow("02", "def object Set: Obj {intr} def obj Test:Set {intr};", 0)>]
    [<DataRow("03", "def obj Set:Obj {intr} def obj Test:Set {intr};", 0)>]
    [<DataRow("04", "def obj Set:Obj {intr} def obj Test:SetTypo {intr};", 0)>] // this should cause the ID010 error only and not SIG04
    [<DataRow("05", "def pred Test(x:Set) {intr};", 1)>]
    [<DataRow("06", "def obj Set:Obj {intr} def pred Test(x:Set) {intr};", 0)>]
    [<DataRow("07", "def obj Set:Obj {intr} def pred Test(x:SetTypo) {intr};", 1)>]
    [<DataRow("08", "def obj Set:Obj {intr} axiom Test {dec ~x:SetTypo; true};", 1)>]
    [<DataRow("09", "def obj Set:Obj {intr} axiom Test {dec ~x:Set; true};", 0)>]
    [<DataRow("10", "def obj Set:Obj {intr} def func PowerSer(x:Set) -> Set {dec ~y:Set; return y};", 0)>]
    [<DataRow("11", "def obj Set:Obj {intr} axiom Test {dec ~x:Set; true};", 0)>]
    [<DataRow("12", "def obj Set:Obj {intr} axiom Test {dec ~x:SetTypo; true};", 1)>]
    ...
    [<DataRow("16", "def obj Set:Obj {intr} def pred Test() {dec ~x:Object; is(x,Set)};", 0)>]
    ...
    [<DataRow("38", """def object Nat: Obj {ctor Nat(){dec self:=x.R(); }};""", 1)>]
    ...
    [<DataRow("44", """def obj A:Obj {intr} def func Add(n,m:A)->A {return self(n,m)};""", 0)>]
    [<DataRow("45", """def obj A:Obj {intr} def func Add(n,m:A)->A {dec ~x:A; return x};""", 0)>]
    [<DataRow("46", """def obj A:Obj {intr} def func Add(n,m:A)->A {dec ~x:A; return x} prop P {dec ~op:Add; true};""", 0)>]
    [<DataRow("47", """def obj A:Obj {intr property pred T() {true} property pred S() {T()}};""", 0)>]
    [<DataRow("48", """def obj A:Obj {dec ~x:Obj; ctor A(y:Obj) {dec base.obj() x:=y; } property func P()->obj {return x}} def pred T(r:A) {r.P()};""", 0)>]
    [<DataRow("49", """def obj A:Obj {ctor A(y:+Obj) {}} def object B:Obj {ctor B(z:+Obj) {dec ~a:A base.obj() a := A(z); }};""", 0)>]
    [<DataRow("50", """def obj A:Obj {intr property pred T() {true}} def obj B:A {ctor B() {dec base.A() assert self.T(); }};""", 0)>]
    [<DataRow("51", """def func A(n,m:Obj)->obj {intr} prop T {dec ~op:A ~x,y:Obj; (op(x,y) = x)};""", 0)>]
    [<DataRow("52", """def obj T:Obj { dec ~x:+tpl; ctor T(y:+tpl) {dec base.obj() x:=y; } property func C(i:ind) -> tpl {return x[i]}};""", 0)>]
    [<DataRow("53", """def obj Nat:Obj {intr} ext D x@/\d+/ -> Nat {dec ~n,m:Nat cases ( | (x = @0) : n:=m ? m:=n ); return n } def func Add()->obj {intr} prop K {dec ~op:Add ~n:Nat; ( op(n,@0) = n ) } ;""", 0)>]
    [<DataRow("54", """def obj C: Obj {ctor C() {dec base.obj ();  }} def pred T() {dec ~cI1:C cI1:=C; true } ;""", 0)>]
    [<DataRow("54a", """def obj C: Obj {ctor C() {dec base.obj ();  }} def pred T() {dec ~cI1:C cI1:=C(); true } ;""", 0)>]
    [<DataRow("54b", """def obj C: Obj {ctor C(x:Obj) {dec base.obj ();  }} def pred T() {dec ~cI1:C cI1:=C(); true } ;""", 1)>]
    [<DataRow("54c", """def obj C: Obj {ctor C(x:Obj) {dec base.obj ();  }} def pred T() {dec ~x:Obj ~cI1:C cI1:=C(x); true } ;""", 0)>]
    [<DataRow("54b_", """def obj C1: Obj {ctor C1(i1:ind) {dec ~o:ind o:=i1 base.obj ();  }} ;""", 0)>]
    [<DataRow("54c_", """def obj C1: Obj {ctor C1(i1:ind) {dec ~o:ind o:=i1 base.obj ();  }} ;""", 0)>]
    [<DataRow("55", """def obj B: A {intr} def obj D: B {ctor D() {dec base.B();  }} def pred T() {dec ~dI1:D dI1:=D; true } ;""", 0)>]
    [<DataRow("55a", """def obj B: A {intr} def obj D: B {ctor D() {dec base.B();  }} def pred T() {dec ~dI1:D dI1:=D(); true } ;""", 0)>]
    [<DataRow("55b", """def obj B: A {intr} def obj D: B {ctor D(x:Obj) {dec base.B();  }} def pred T() {dec ~dI1:D dI1:=D; true } ;""", 1)>]
    [<DataRow("55c", """def obj B: A {intr} def obj D: B {ctor D(x:Obj) {dec base.B();  }} def pred T() {dec ~dI1:D dI1:=D(); true } ;""", 1)>]
    ...
    [<DataRow("inh", """def obj A:Obj { intr } def pred T() {dec ~n:A n:=A(); true};""", 0)>]
    [<DataRow("inh_a", """def obj A:Obj { intr } def pred T() {dec ~n:Obj n:=A(); true};""", 0)>]
    [<DataRow("inh_b", """def obj A:Obj {intr} def obj B:A { intr } def pred T() {dec ~n:A n:=B(); true};""", 0)>]
    [<DataRow("inh_c", """def obj A:Obj {intr} def obj B:A { intr } def pred T() {dec ~n:B n:=A(); true};""", 1)>]
    [<DataRow("inh_d", """def obj A:Obj {intr} def obj B:A { intr } def pred T() {dec ~n:Obj n:=B(); true};""", 0)>]
    [<DataRow("inh_e", """def obj A:Obj {intr} def obj B:A { intr } def pred T() {dec ~n:Obj n:=A(); true};""", 0)>]
    [<DataRow("inh_f", """def obj A:Obj {intr} def obj B:Obj { intr } def pred T() {dec ~n:B n:=A(); true};""", 1)>]
    [<DataRow("inh_g", """def obj A:Obj {intr} def obj B:Obj { intr } def pred T() {dec ~n:A n:=B(); true};""", 1)>]
    [<DataRow("inh_type_a", """def obj A:Obj { intr } def pred T() {dec ~n:ind n:=A(); true};""", 1)>]
    [<DataRow("inh_type_b", """def obj A:Obj { intr } def pred T() {dec ~n:pred n:=A(); true};""", 1)>]
    [<DataRow("inh_type_c", """def obj A:Obj { intr } def pred T() {dec ~n:func n:=A(); true};""", 1)>]
    [<DataRow("constr_a", """def obj A:Obj { intr } def obj B: A {intr} def obj D: B {ctor D() {dec base.B();  }} def pred T() {dec ~dI2:D dI2:=D(); true};""", 0)>]
    [<DataRow("constr_b", """def obj A:Obj { intr } def obj B: A {intr} def obj D: B {ctor D() {dec base.B();  }} def pred T() {dec ~dI2:B dI2:=D(); true};""", 0)>]
    [<DataRow("constr_c", """def obj A:Obj { intr } def obj B: A {intr} def obj D: B {ctor D() {dec base.B();  }} def pred T() {dec ~dI2:A dI2:=D(); true};""", 0)>]
    [<DataRow("constr_d", """def obj A:Obj { intr } def obj B: A {intr} def obj D: B {ctor D() {dec base.B();  }} def pred T() {dec ~dI2:Obj dI2:=D(); true};""", 0)>]
    [<DataRow("constr_inh_a", """def obj A:Obj { ctor A(x:Obj) {dec base.obj(); } } def obj B:A { ctor B(x:pred) {dec base.A(); } } def pred T() {dec ~n:A n:=B(); true};""", 1)>]
    [<DataRow("constr_inh_b", """def obj A:Obj { ctor A(x:Obj) {dec base.obj(); } } def obj B:A { ctor B(x:pred) {dec base.A(); } } def pred T() {dec ~n:Obj n:=B(); true};""", 1)>]
    [<DataRow("constr_inh_c", """def obj A:Obj { ctor A(x:Obj) {dec base.obj(); } } def obj B:A { ctor B(x:pred) {dec base.A(); } } def pred T() {dec ~n:Obj n:=A(); true};""", 1)>]
    [<DataRow("constr_inh_d", """def obj A:Obj { ctor A(x:Obj) {dec base.obj(); } } def obj B:A { ctor B(x:pred) {dec base.A(); } } def pred T() {dec ~n:B ~x:Obj n:=A(x); true};""", 1)>]
    [<DataRow("constr_inh_e", """def obj A:Obj { ctor A(x:Obj) {dec base.obj(); } } def obj B:Obj { ctor B(x:pred) {dec base.obj(); } } def pred T() {dec ~n:B ~x:Obj n:=A(x); true};""", 1)>]
    [<DataRow("constr_inh_f", """def obj A:Obj { ctor A(x:Obj) {dec base.obj(); } } def obj B:Obj { ctor B(x:pred) {dec base.obj(); } } def pred T() {dec ~n:A ~x:pred n:=B(x); true};""", 1)>]
    [<DataRow("ass_ind_ind", """def obj C1: Obj {ctor C1(i1:ind) {dec ~o:ind o:=i1 base.obj ();  }} ;""", 0)>]
    [<DataRow("ass_ind_pred", """def obj C1: Obj {ctor C1(i1:pred) {dec ~o:ind o:=i1 base.obj ();  }};""", 1)>]
    [<DataRow("ass_ind_func", """def obj C1: Obj {ctor C1(i1:func) {dec ~o:ind o:=i1 base.obj ();  }};""", 1)>]
    [<DataRow("ass_ind_obj", """def obj C1: Obj {ctor C1(i1:Obj) {dec ~o:ind o:=i1 base.obj ();  }};""", 1)>]
    [<DataRow("ass_pred_ind", """def obj C1: Obj {ctor C1(i1:ind) {dec ~o:pred o:=i1 base.obj ();  }} ;""", 1)>]
    [<DataRow("ass_pred_pred", """def obj C1: Obj {ctor C1(i1:pred) {dec ~o:pred o:=i1 base.obj ();  }};""", 0)>]
    [<DataRow("ass_pred_func", """def obj C1: Obj {ctor C1(i1:func) {dec ~o:pred o:=i1 base.obj ();  }};""", 1)>]
    [<DataRow("ass_pred_obj", """def obj C1: Obj {ctor C1(i1:Obj) {dec ~o:pred o:=i1 base.obj ();  }};""", 1)>]
    [<DataRow("ass_func_ind", """def obj C1: Obj {ctor C1(i1:ind) {dec ~o:func o:=i1 base.obj ();  }} ;""", 1)>]
    [<DataRow("ass_func_pred", """def obj C1: Obj {ctor C1(i1:pred) {dec ~o:func o:=i1 base.obj ();  }};""", 1)>]
    [<DataRow("ass_func_func", """def obj C1: Obj {ctor C1(i1:func) {dec ~o:func o:=i1 base.obj ();  }};""", 0)>]
    [<DataRow("ass_func_obj", """def obj C1: Obj {ctor C1(i1:Obj) {dec ~o:func o:=i1 base.obj ();  }};""", 1)>]
    [<DataRow("ass_obj_ind", """def obj C1: Obj {ctor C1(i1:ind) {dec ~o:Obj o:=i1 base.obj ();  }} ;""", 1)>]
    [<DataRow("ass_obj_pred", """def obj C1: Obj {ctor C1(i1:pred) {dec ~o:Obj o:=i1 base.obj ();  }};""", 1)>]
    [<DataRow("ass_obj_func", """def obj C1: Obj {ctor C1(i1:func) {dec ~o:Obj o:=i1 base.obj ();  }};""", 1)>]
    [<DataRow("ass_obj_obj", """def obj C1: Obj {ctor C1(i1:Obj) {dec ~o:Obj o:=i1 base.obj ();  }};""", 0)>]
    [<DataRow("99", "uses Fpl.Commons.Structures ;", 0)>]
    [<TestMethod>]
    ...
    [<DataRow("00a", "def obj A:Obj { intr prty pred T() {intr} } def obj B:Obj { intr prty pred T() {intr} } def obj C:A,B { intr } ;", 1)>]
    [<DataRow("00b", "def obj A:Obj { intr prty pred T() {intr} } def obj B:Obj { intr prty pred S() {intr} } def obj C:A,B { intr } ;", 0)>]
    [<DataRow("00c", "def obj A:Obj { intr prty pred T() {intr} } def obj B:Obj { intr prty pred T(x:func) {intr} } def obj C:A,B { intr } ;", 0)>]
    [<DataRow("01a", "def obj A:Obj { intr prty func T()->obj {intr} } def obj B:Obj { intr prty func T()->obj {intr} } def obj C:A,B { intr } ;", 1)>]
    [<DataRow("01b", "def obj A:Obj { intr prty func T()->obj {intr} } def obj B:Obj { intr prty pred T() {intr} } def obj C:A,B { intr } ;", 0)>]
    [<DataRow("01c", "def obj A:Obj { intr prty func T()->obj {intr} } def obj B:Obj { intr prty func T(x:ind)->obj {intr} } def obj C:A,B { intr } ;", 0)>]
    [<DataRow("99", "uses Fpl.Commons.Structures ;", 0)>]
    [<TestMethod>]
    ...
    [<DataRow("14", "def object A: Obj {ctor A(x: Obj, p:pred(u: pred)) {dec assert u;  }};", 0)>]
    ...
    [<DataRow("00_2", "def obj Test:Obj {dec ~x,x:* pred; ctor Test(){}};", 1)>]
    [<DataRow("00a_2", "def obj Test:Obj {dec ~x,x:+ pred; ctor Test(){}};", 1)>]
    [<DataRow("00b_2", "def obj Test:Obj {dec ~x,x: pred; ctor Test(){}};", 1)>]
    [<DataRow("00c_2", "def obj Test:Obj {dec ~x: pred; ctor Test(){}};", 0)>]
    ...
    [<DataRow("10", "def obj Test:Obj {dec ~x:ind; ctor Test(x: pred) {} };", 1)>]
    [<DataRow("10a", "def obj Test:Obj {dec ~x:ind; ctor Test(x:* pred) {} };", 1)>]
    [<DataRow("10b", "def obj Test:Obj {dec ~x:ind; ctor Test(x:+ pred) {} };", 1)>]
    [<DataRow("10c", "def obj Test:Obj {dec ~x:ind; ctor Test() {dec ~x: Obj; } };", 1)>]
    [<DataRow("10d", "def obj Test:Obj {dec ~x:ind; ctor Test() {dec ~x:* obj; } };", 1)>]
    [<DataRow("10e", "def obj Test:Obj {dec ~x:ind; ctor Test() {dec ~x:+ obj; } };", 1)>]
    [<DataRow("11", "def obj Test:Obj {dec ~x:Obj; constructor Test() {} prty func X(x: pred)->obj {intr} };", 1)>]
    [<DataRow("11a", "def obj Test:Obj {dec ~x:Obj; constructor Test(x: pred) {} prty func X()->obj {intr} };", 1)>]
    [<DataRow("11b", "def obj Test:Obj {dec ~x:Obj; constructor Test() {dec ~x: pred; } prty func X()->obj {intr} };", 1)>]
    [<DataRow("11c", "def obj Test:Obj {dec ~x:Obj; constructor Test() {} prty func X()->obj {dec ~x: pred; return x} };", 1)>]
    ...
    [<DataRow("14", """def obj B:Obj {intr} def obj A:Obj {dec ~x:Obj; ctor A(y:B) {} };""", 1)>]
    ...
    [<DataRow("03", "def obj T:Obj {dec ~x:Obj; ctor T() {}};", 1)>]
    [<DataRow("03a", "def obj T:Obj {dec ~x:Obj; ctor T() {dec x:=x; }};", 0)>]
    [<DataRow("03b", "def obj T:Obj {dec ~x:Obj; ctor T() {dec y:=y; }};", 1)>]
    ...
    [<DataRow("00a", "def obj T:Obj { dec ~x:Obj; ctor T() { dec base.obj() ; }} def obj S:T { dec ~x:Obj; ctor S() { dec base.T() ; }} ;", 1)>]
    [<DataRow("00b", "def obj T:Obj { dec ~x:Obj; ctor T() { dec base.obj() ; }} def obj S:T { dec ~y:Obj; ctor S() { dec base.T() ; }} ;", 0)>]
    [<DataRow("99", "uses Fpl.Commons.Structures ;", 0)>]
    [<TestMethod>]
