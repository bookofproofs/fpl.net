...
type TestInterpreterErrors() =
    ...
    [<DataRow("""def pred T (x:tplTest) {true} def pred Caller() {dec ~x:ind; T(x)} ;""", 0)>]
    [<DataRow("""def pred T (x,y,z:obj) {true} def pred Caller() {dec ~x:obj; T(x)} ;""", 1)>]
    [<DataRow("""def pred T (x,y,z:obj) {true} def pred Caller() {dec ~x:obj; T(x,x)} ;""", 1)>]
    [<DataRow("""def pred T (x,y,z:obj) {true} def pred Caller() {dec ~x:obj; T(x,x,x)} ;""", 0)>]
    ...
    [<DataRow("""def func T(y:obj)->obj { return self(y)} ;""", 0)>]
    [<DataRow("""def func T(y:obj)->obj { intr } def func S()->obj {dec ~x:obj; return T(x)} ;""", 0)>]
    [<DataRow("""axiom T() { dec ~x:obj; all p:pred(y:obj) {p(x)}};""", 0)>]
    [<DataRow("""def cl A:obj {intr} def func Add(n,m:A)->A {dec ~x,y:A; return self(x,y)};""", 0)>]
    [<DataRow("""def cl A:obj {intr} def func Add(n,m:A)->A {dec ~x:A; return x};""", 0)>]
    [<DataRow("""def cl A:obj {intr} def func Add(n,m:A)->A {dec ~x:A; return x} prop P(op:Add) {true};""", 0)>]
    [<DataRow("uses Fpl.Commons.Structures ;", 0)>]
    [<TestMethod>]
