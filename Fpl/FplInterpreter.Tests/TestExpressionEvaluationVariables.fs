...
type TestExpressionEvaluationVariables() =
    ...
    [<DataRow("def pred T() { dec x,y,z:pred x:=true z:=true; and(x,and(y,z)) }", LiteralUndet)>]
    [<DataRow("def pred T() { dec x,y,z:pred x:=true y:=true; and(x,and(y,z)) }", LiteralUndet)>]
    [<DataRow("def pred T() { dec x,y,z:pred y:=true z:=true; and(x,and(y,z)) }", LiteralUndet)>]
    ...
    [<DataRow("def pred T() { dec x,y,z:pred x:=false z:=false; or(x,or(y,z)) }", LiteralUndet)>]
    [<DataRow("def pred T() { dec x,y,z:pred x:=false y:=false; or(or(x,y),z) }", LiteralUndet)>]
    [<DataRow("def pred T() { dec x,y,z:pred y:=false z:=false; or(x,or(y,z)) }", LiteralUndet)>]
    ...
    [<DataRow("def pred T() { dec x,y,z:pred x:=false y:=true; xor(xor(x,y),z) }", LiteralUndet)>]
    [<DataRow("def pred T() { dec x,y,z:pred y:=false z:=true; xor(x,xor(y,z)) }", LiteralUndet)>]
    [<DataRow("def pred T() { dec x,y,z:pred x:=true z:=false; xor(x,xor(y,z)) }", LiteralUndet)>]
    ...
    [<DataRow("def pred T() { dec x,y:pred y:=false; impl(x,y) }", LiteralUndet)>]
    [<DataRow("def pred T() { dec x,y:pred x:=false; impl(x,y) }", LiteralUndet)>]
    [<DataRow("def pred T() { dec x,y:pred y:=true; impl(x,y) }", LiteralUndet)>]
    [<DataRow("def pred T() { dec x,y:pred x:=true; impl(x,y) }", LiteralUndet)>]
    [<TestMethod>]
    ...
    [<DataRow("def pred T() { dec x,y:pred y:=false; iif(x,y) }", LiteralUndet)>]
    [<DataRow("def pred T() { dec x,y:pred x:=false; iif(x,y) }", LiteralUndet)>]
    [<DataRow("def pred T() { dec x,y:pred y:=true; iif(x,y) }", LiteralUndet)>]
    [<DataRow("def pred T() { dec x,y:pred x:=true; iif(x,y) }", LiteralUndet)>]
    [<TestMethod>]
