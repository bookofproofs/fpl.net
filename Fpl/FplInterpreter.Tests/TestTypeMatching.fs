...
type TestTypeMatching() =
    ...
    [<DataRow("pred_obj_01", "def pred T(a:pred(b:obj)) {true};", 1)>]
    [<DataRow("pred_pred_01", "def pred T(a:pred(b:pred)) {true};", 1)>]
    [<DataRow("01", "def pred T(a:pred(b:ind)) {dec ~x:pred(y:obj); x};", 1)>]
    [<TestMethod>]
