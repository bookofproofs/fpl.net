...
type TestTypeMatching() =
    ...
    [<DataRow("DisjunctiveSyllogism_02", "inf DisjunctiveSyllogism{dec ~p,q:pred; pre:not p,or(p,q) con:q} thm T {true} proof T$1 {1. |- not ex x:obj { is(x,N) } 2. |- or(iif(true,false), xor(true,false)) 2. 1, 2, byinf DisjunctiveSyllogism |- true };")>]
    ...
    member this.TestGetOpenFormulaOfExpressionProofs(no:string, fplCode) =
        let filename = "TestGetOpenFormulaOfExpressionProofs.fpl"
        prepareFplCode(filename + ".fpl", fplCode, false) 
        let r = heap.Root
        let theory = r.Scope[filename]
        // expected value
