...
type TestFplValueScopeFplId() =
    ...
    member this.TestBlocks(var) =
        let res = TestCases.ScopeBlocks("FplId") 
        match res with
    ...
    member this.TestConstructors(var) =
        let res = TestCases.ScopeConstructors("FplId") 
        match res with
    ...
    member this.TestProofsAndCorollaries(var) =
        let res = TestCases.ScopeProofsAndCorollaries("FplId") 
        match res with
    ...
    member this.TestProperties(var) =
        let res = TestCases.ScopeProperties("FplId") 
        match res with
    ...
    member this.TestVariablesInBlock(var) =
        let result = TestCases.ScopeVariablesInBlock("FplId")
        match result with
    ...
    member this.TestVariablesInBlockVariadic(var) =
        let result = TestCases.ScopeVariablesInBlockVariadic("FplId")
        match result with
    ...
    member this.TestVariablesInSignature(var) =
        let result = TestCases.ScopeVariablesInSignature("FplId")
        match result with
        | Some (r,theory,block,x,y,xw,xu,xv,yw,yu,yv,xwa,xwb,xwc,xua,xub,xuc,xva,xvb,xvc,ywa,ywb,ywc,yua,yub,yuc,yva,yvb,yvc) ->
            match var with
            | "r" -> Assert.AreEqual<string>("", r.FplId)
    ...
    member this.TestVariablesInSignatureVariadic(var) =
        let result = TestCases.ScopeVariablesInSignatureVariadic("FplId")
        match result with
        | Some (r,theory,block,x,y,xw,xu,xv,yw,yu,yv,xwa,xwb,xwc,xua,xub,xuc,xva,xvb,xvc,ywa,ywb,ywc,yua,yub,yuc,yva,yvb,yvc) ->
            match var with
            | "r" -> Assert.AreEqual<string>("", r.FplId)
    ...
    [<TestMethod>]
    member this.TestLanguageFplId(var, predName, predDecl, trslCode) =
    ...
    [<DataRow("base5", "NotEqual(undef, undef)", "NotEqual(x, y)", """!tex: x "\neq" y !eng: x "is unequal" y !ger: x "ist ungleich" y !pol: x ( "nie równa się" | "nie równe" ) y""")>]
    [<TestMethod>]
    member this.TestLocalizationFplId(var, predName, predDecl, trslCode) =
    ...
    [<DataRow("base5", "NotEqual(undef, undef)", "NotEqual(x, y)", """!tex: x "\neq" y !eng: x "is unequal" y !ger: x "ist ungleich" y !pol: x ( "nie równa się" | "nie równe" ) y""")>]
    [<TestMethod>]
    member this.TestTranslationFplId(var, predName, predDecl, trslCode) =
    ...
    member this.TestIntrinsicPrimitives(var) =
        let res = TestCases.ScopeIntrinsicPrimitives("FplId") 
        match res with
