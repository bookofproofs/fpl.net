...
type TestPR010() =
    ...
    [<DataRow("02a", "ax A {true} thm T {true} proof T$1 {1. byax A |- trivial}", 0)>]
    [<DataRow("03a", "conj A {true} thm T {true} proof T$1 {1. byconj A |- trivial}", 0)>]
    [<DataRow("04a", "def pred A() {true} thm T {true} proof T$1 {1. bydef A |- trivial}", 0)>]
    [<DataRow("05a", "inf A {pre: true con: true} thm T {true} proof T$1 {1. byinf A |- trivial}", 0)>]
