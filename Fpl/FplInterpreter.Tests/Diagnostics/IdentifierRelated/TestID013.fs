...
type TestID013() =
    ...
    [<DataRow("16", """def pred Equal(x,y: tpl) infix "=" 0 {del.Equal(x,y)}""", 1, "missing error message")>]
    [<DataRow("16a", """def pred Equal(x,y: ind) infix "=" 0 {del.Equal(x,y)}""", 1, "missing error message")>]
    [<DataRow("16b", """def pred Equal(x,y: pred) infix "=" 0 {del.Equal(x,y)}""", 1, "missing error message")>]
    [<DataRow("16c", """def pred Equal(x,y: obj) infix "=" 0 {del.Equal(x,y)}""", 1, "missing error message")>]
    [<DataRow("16d", """def pred Equal(x,y: obj) infix "=" 0 {del.Equal(x,y)}""", 1, "missing error message")>]
    [<DataRow("16e", """def cl A def pred Equal(x,y: A) infix "=" 0 {del.Equal(x,y)}""", 1, "missing error message")>]
