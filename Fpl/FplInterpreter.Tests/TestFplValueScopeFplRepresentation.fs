...
type TestFplValueScopeFplRepresentation() =
    ...
    member this.TestPredicate(var, varVal) =
        ...
        | Some st -> 
            ...
            | "base16" -> Assert.AreEqual<string>("undef(undef(*(=(+(undef, undef), 2), undef){undef(+(undef, undef){undef, undef}, 2), undef}))", base1.Type(SignatureType.Repr))
            | "base17" -> Assert.AreEqual<string>("undef(undef(*(=(+(undef, '(undef)), 2), undef){undef(+(undef, '(undef)){undef, undef(undef(undef))}, 2), undef}))", base1.Type(SignatureType.Repr))
