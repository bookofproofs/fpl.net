...
type TestRepresentation() =
    ...
    [<DataRow("pr03", "(undef = undef)", LiteralUndet)>]
    [<DataRow("pr04", "(true = true)", LiteralTrue)>]
    [<DataRow("pr05", "(true = undef)", LiteralUndet)>]
    [<DataRow("pr06", "(undef = true)", LiteralUndet)>]
    [<DataRow("m01", "(@3 = $3)", LiteralFalse)>]
    [<DataRow("m02", "(a = true)", LiteralUndet)>]
    [<DataRow("m03", "(i = $3)", LiteralUndet)>]
    [<DataRow("m04", "(j = $2)", LiteralTrue)>]
    [<DataRow("m05", "(k = $2)", LiteralFalse)>]
    [<DataRow("m06", "(true = a)", LiteralUndet)>]
    [<DataRow("m07", "($3 = i)", LiteralUndet)>]
    ...
    [<DataRow("00","""def pred T() { dec v:ind; true }""", LiteralUndet)>]
    [<DataRow("00a","""def pred T() { dec v:pred; true }""", LiteralUndet)>]
    [<DataRow("00b","""def pred T() { dec v:obj; true }""", LiteralUndet)>]
    [<DataRow("00c","""def pred T() { dec v:func; true }""", LiteralUndet)>]
    [<DataRow("00d","""def pred T() { dec v:tpl; true }""", LiteralUndet)>]
    [<DataRow("01a","""def pred T() { dec v:pred(d:ind); true }""", LiteralUndet)>]
    [<DataRow("01b","""def pred T() { dec v:pred(d:pred(e,f:obj)); true }""", LiteralUndet)>]
    [<DataRow("01c","""def pred T() { dec v:func(x:pred(y:obj,d,e:ind)) ->pred(i:pred(j,k:obj)); true }""", LiteralUndet)>]
    [<DataRow("02","""def pred T() { dec v:A; true }""", LiteralUndet)>]
    [<DataRow("02a","""def cl A {intr} def pred T() { dec v:A; true }""", LiteralUndet)>]
    [<TestMethod>]
