...
type TestFplValue() =
    ...
    [<DataRow("def pred TestId(x:*ind[ind]) {true};", "TestId(*ind)", "pred(*ind)")>]
    [<DataRow("def pred TestId(x:*pred[ind]) {true};", "TestId(+pred)", "pred(+pred)")>]
    [<DataRow("def pred TestId(x:*func[ind]) {true};", "TestId(*func)", "pred(*func)")>]
    [<DataRow("def pred TestId(x:*obj[ind]) {true};", "TestId(+obj)", "pred(+obj)")>]
    [<DataRow("def pred TestId(x:*index[ind]) {true};", "TestId(+ind)", "pred(+ind)")>]
    [<DataRow("def pred TestId(x:*predicate[ind]) {true};", "TestId(*pred)", "pred(*pred)")>]
    [<DataRow("def pred TestId(x:*function[ind]) {true};", "TestId(+func)", "pred(+func)")>]
    [<DataRow("def pred TestId(x:*object[ind]) {true};", "TestId(*obj)", "pred(*obj)")>]
    [<DataRow("def pred TestId(x:*Nat) {true};", "TestId(+Nat)", "pred(+Nat)")>]
    [<DataRow("def pred TestId(x:*tpl) {true};", "TestId(*tpl)", "pred(*tpl)")>]
    [<DataRow("def pred TestId(x:*template[ind]) {true};", "TestId(+template)", "pred(+template)")>]
    [<DataRow("def pred TestId(x:*tplTest[ind]) {true};", "TestId(*tplTest)", "pred(*tplTest)")>]
    [<DataRow("def pred TestId(x:*templateTest[ind]) {true};", "TestId(+templateTest)", "pred(+templateTest)")>]
    [<DataRow("def pred TestId(x,y,z:*obj[ind]) {true};", "TestId(+obj, +obj, +obj)", "pred(+obj, +obj, +obj)")>]
    [<DataRow("def pred TestId(x,y:*pred(z:obj)[ind]) {true};", "TestId(+pred(obj), +pred(obj))", "pred(+pred(obj), +pred(obj))")>]
    [<DataRow("def pred TestId(x,y:pred(u,v,w:*obj[ind])) {true};", "TestId(pred(*obj, *obj, *obj), pred(*obj, *obj, *obj))", "pred(pred(*obj, *obj, *obj), pred(*obj, *obj, *obj))")>]
    [<DataRow("def pred TestId(x:func(u:*obj[ind])->Nat) {true};", "TestId(func(+obj) -> Nat)", "pred(func(+obj) -> Nat)")>]
    ...
    [<DataRow("def func TestId(x:*ind[ind]) -> obj {intrinsic};", "TestId(*ind) -> obj", "func(*ind) -> obj")>]
    [<DataRow("def func TestId(x:*pred[ind]) -> obj {intrinsic};", "TestId(+pred) -> obj", "func(+pred) -> obj")>]
    [<DataRow("def func TestId(x:*func[ind]) -> obj {intrinsic};", "TestId(*func) -> obj", "func(*func) -> obj")>]
    [<DataRow("def func TestId(x:*obj[ind]) -> obj {intrinsic};", "TestId(+obj) -> obj", "func(+obj) -> obj")>]
    [<DataRow("def func TestId(x:*index[ind]) -> obj {intrinsic};", "TestId(+ind) -> obj", "func(+ind) -> obj")>]
    [<DataRow("def func TestId(x:*predicate[ind]) -> obj {intrinsic};", "TestId(*pred) -> obj", "func(*pred) -> obj")>]
    [<DataRow("def func TestId(x:*function[ind]) -> obj {intrinsic};", "TestId(+func) -> obj", "func(+func) -> obj")>]
    [<DataRow("def func TestId(x:*object) -> obj {intrinsic};", "TestId(*obj) -> obj", "func(*obj) -> obj")>]
    [<DataRow("def func TestId(x:*Nat[ind]) -> obj {intrinsic};", "TestId(+Nat) -> obj", "func(+Nat) -> obj")>]
    [<DataRow("def func TestId(x:*Nat[ind]) -> obj {intrinsic};", "TestId(*Nat) -> obj", "func(*Nat) -> obj")>]
    [<DataRow("def func TestId(x:*tpl[ind]) -> obj {intrinsic};", "TestId(*tpl) -> obj", "func(*tpl) -> obj")>]
    [<DataRow("def func TestId(x:*template[ind]) -> obj {intrinsic};", "TestId(+template) -> obj", "func(+template) -> obj")>]
    [<DataRow("def func TestId(x:*tplTest) -> obj {intrinsic};", "TestId(*tplTest) -> obj", "func(*tplTest) -> obj")>]
    [<DataRow("def func TestId(x:*templateTest[ind]) -> obj {intrinsic};", "TestId(+templateTest) -> obj", "func(+templateTest) -> obj")>]
    [<DataRow("def func TestId(x,y,z:*obj[ind]) -> obj {intrinsic};", "TestId(*obj[ind], *obj[ind], *obj[ind]) -> obj", "func(+obj, +obj, +obj) -> obj")>]
    [<DataRow("def func TestId(x,y:*pred(z:obj)[ind]) -> obj {intrinsic};", "TestId(+pred(obj), +pred(obj)) -> obj", "func(+pred(obj), +pred(obj)) -> obj")>]
    [<DataRow("def func TestId(x,y:pred(u,v,w:*obj)) -> obj {intrinsic};", "TestId(pred(*obj, *obj, *obj), pred(*obj, *obj, *obj)) -> obj", "func(pred(*obj, *obj, *obj), pred(*obj, *obj, *obj)) -> obj")>]
    [<DataRow("def func TestId(x:func(u:*obj[ind])->Nat) -> obj {intrinsic};", "TestId(func(+obj) -> Nat) -> obj", "func(func(+obj) -> Nat) -> obj")>]
    ...
    [<DataRow("def cl T {intr prty pred TestId(x:*ind[ind]) {intrinsic}};", "TestId(*ind)", "pred(*ind)")>]
    [<DataRow("def cl T {intr prty pred TestId(x:*pred[ind]) {intrinsic}};", "TestId(+pred)", "pred(+pred)")>]
    [<DataRow("def cl T {intr prty pred TestId(x:*func[ind]) {intrinsic}};", "TestId(*func)", "pred(*func)")>]
    [<DataRow("def cl T {intr prty pred TestId(x:*obj[ind]) {intrinsic}};", "TestId(+obj)", "pred(+obj)")>]
    [<DataRow("def cl T {intr prty pred TestId(x:*index[ind]) {intrinsic}};", "TestId(+ind)", "pred(+ind)")>]
    [<DataRow("def cl T {intr prty pred TestId(x:*predicate[ind]) {intrinsic}};", "TestId(*pred)", "pred(*pred)")>]
    [<DataRow("def cl T {intr prty pred TestId(x:*function[ind]) {intrinsic}};", "TestId(+func)", "pred(+func)")>]
    [<DataRow("def cl T {intr prty pred TestId(x:*object) {intrinsic}};", "TestId(*obj)", "pred(*obj)")>]
    [<DataRow("def cl T {intr prty pred TestId(x:*Nat[ind]) {intrinsic}};", "TestId(+Nat)", "pred(+Nat)")>]
    [<DataRow("def cl T {intr prty pred TestId(x:*Nat[ind]) {intrinsic}};", "TestId(*Nat)", "pred(*Nat)")>]
    [<DataRow("def cl T {intr prty pred TestId(x:*tpl[ind]) {intrinsic}};", "TestId(*tpl)", "pred(*tpl)")>]
    [<DataRow("def cl T {intr prty pred TestId(x:*template[ind]) {intrinsic}};", "TestId(+template)", "pred(+template)")>]
    [<DataRow("def cl T {intr prty pred TestId(x:*tplTest[ind]) {intrinsic}};", "TestId(*tplTest)", "pred(*tplTest)")>]
    [<DataRow("def cl T {intr prty pred TestId(x:*templateTest[ind]) {intrinsic}};", "TestId(+templateTest)", "pred(+templateTest)")>]
    [<DataRow("def cl T {intr prty pred TestId(x,y,z:*obj[ind]) {intrinsic}};", "TestId(+obj, +obj, +obj)", "pred(+obj, +obj, +obj)")>]
    [<DataRow("def cl T {intr prty pred TestId(x,y:*pred(z:obj)[ind]) {intrinsic}};", "TestId(+pred(obj), +pred(obj))", "pred(+pred(obj), +pred(obj))")>]
    [<DataRow("def cl T {intr prty pred TestId(x,y:pred(u,v,w:*obj)) {intrinsic}};", "TestId(pred(*obj, *obj, *obj), pred(*obj, *obj, *obj))", "pred(pred(*obj, *obj, *obj), pred(*obj, *obj, *obj))")>]
    [<DataRow("def cl T {intr prty pred TestId(x:func(u:*obj[ind])->Nat) {intrinsic}};", "TestId(func(+obj) -> Nat)", "pred(func(+obj) -> Nat)")>]
    ...
    [<DataRow("def cl T {intr prty func TestId(x:*ind[ind]) -> obj {intrinsic}};", "TestId(*ind) -> obj", "func(*ind) -> obj")>]
    [<DataRow("def cl T {intr prty func TestId(x:*pred[ind]) -> obj {intrinsic}};", "TestId(+pred) -> obj", "func(+pred) -> obj")>]
    [<DataRow("def cl T {intr prty func TestId(x:*func[ind]) -> obj {intrinsic}};", "TestId(*func) -> obj", "func(*func) -> obj")>]
    [<DataRow("def cl T {intr prty func TestId(x:*obj[ind]) -> obj {intrinsic}};", "TestId(+obj) -> obj", "func(+obj) -> obj")>]
    [<DataRow("def cl T {intr prty func TestId(x:*index[ind]) -> obj {intrinsic}};", "TestId(+ind) -> obj", "func(+ind) -> obj")>]
    [<DataRow("def cl T {intr prty func TestId(x:*predicate[ind]) -> obj {intrinsic}};", "TestId(*pred) -> obj", "func(*pred) -> obj")>]
    [<DataRow("def cl T {intr prty func TestId(x:*function[ind]) -> obj {intrinsic}};", "TestId(+func) -> obj", "func(+func) -> obj")>]
    [<DataRow("def cl T {intr prty func TestId(x:*object[ind]) -> obj {intrinsic}};", "TestId(*obj) -> obj", "func(*obj) -> obj")>]
    [<DataRow("def cl T {intr prty func TestId(x:*Nat[ind]) -> obj {intrinsic}};", "TestId(+Nat) -> obj", "func(+Nat) -> obj")>]
    [<DataRow("def cl T {intr prty func TestId(x:*Nat[ind]) -> obj {intrinsic}};", "TestId(*Nat) -> obj", "func(*Nat) -> obj")>]
    [<DataRow("def cl T {intr prty func TestId(x:*tpl[ind]) -> obj {intrinsic}};", "TestId(*tpl) -> obj", "func(*tpl) -> obj")>]
    [<DataRow("def cl T {intr prty func TestId(x:*template[ind]) -> obj {intrinsic}};", "TestId(+template) -> obj", "func(+template) -> obj")>]
    [<DataRow("def cl T {intr prty func TestId(x:*tplTest[ind]) -> obj {intrinsic}};", "TestId(*tplTest) -> obj", "func(*tplTest) -> obj")>]
    [<DataRow("def cl T {intr prty func TestId(x:*templateTest[ind]) -> obj {intrinsic}};", "TestId(+templateTest) -> obj", "func(+templateTest) -> obj")>]
    [<DataRow("def cl T {intr prty func TestId(x,y,z:*obj[ind]) -> obj {intrinsic}};", "TestId(+obj, +obj, +obj) -> obj", "func(+obj, +obj, +obj) -> obj")>]
    [<DataRow("def cl T {intr prty func TestId(x,y:*pred(z:obj)[ind]) -> obj {intrinsic}};", "TestId(+pred(obj), +pred(obj)) -> obj", "func(+pred(obj), +pred(obj)) -> obj")>]
    [<DataRow("def cl T {intr prty func TestId(x,y:pred(u,v,w:*obj[ind])) -> obj {intrinsic}};", "TestId(pred(*obj, *obj, *obj), pred(*obj, *obj, *obj)) -> obj", "func(pred(*obj, *obj, *obj), pred(*obj, *obj, *obj)) -> obj")>]
    [<DataRow("def cl T {intr prty func TestId(x:func(u:*obj[ind])->Nat) -> obj {intrinsic}};", "TestId(func(+obj) -> Nat) -> obj", "func(func(+obj) -> Nat) -> obj")>]
    ...
    [<DataRow("def cl T {ctor T(x:*obj[ind]) {}};", "T(*ob[ind]j)", "T")>]
    ...
    [<DataRow("def cl T {ctor T(x,y:*pred(z:obj)) {}};", "T(*pred(obj)[ind], *pred(obj)[ind])", "T")>]
