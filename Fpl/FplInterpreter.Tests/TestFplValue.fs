...
type TestFplValue() =
    ...
    [<DataRow("def pred TestId(x:*Nat[ind]) {true};", "TestId(*at[ind])", "pred(*Nat[ind])")>]
    ...
    [<DataRow("def cl T {intr prty pred TestId(x:*templateTest[ind]) {intrinsic}};", "TestId(*templateTest[ind])", "pred(+templateTest)")>]
