...
type TestFplValueScopeQualifiedName() =
    ...
    member this.TestPredicate(var, varVal) =
        ...
        | "base18" -> Assert.AreEqual<string>("an exists quantor TestPredicateQualifiedName.T1().Ǝx:pred(T), y:C, z:obj (and(a, and(b, c)))", qualifiedName base1 false) 
        | "base19" -> Assert.AreEqual<string>("an exists n times quantor TestPredicateQualifiedName.T1().Ǝ!x:obj (∀y:N (true))" , qualifiedName base1 false) 
        | "base20" -> Assert.AreEqual<string>("an all quantor TestPredicateQualifiedName.T1().∀x:obj (not(x))", qualifiedName base1 false) 
