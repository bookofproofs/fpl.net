module Fpl3LanguageServer.ServiceAutoCompletion.Main
...
open Fpl3LanguageServer.ServiceAutoCompletion.ItemChoices
...
let getParserChoices (builder: StringBuilder) (index: int) (languageServer: ILanguageServer) : Async<CompletionList> =
    async {
        ...
        return CompletionList(modChoices |> Seq.map (fun ci -> ci :> CompletionItem))
    }
