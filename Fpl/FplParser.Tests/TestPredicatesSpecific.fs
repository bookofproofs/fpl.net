...
type TestPredicatesSpecific () =
    ...
    member this.TestAll6 () =
        let result = run (all .>> eof) """all x in Real, y in pred, z in func (and (a,b,c))"""
        let actual = sprintf "%O" result
        printf "%O" actual
        Assert.IsTrue(actual.StartsWith("Success:"))
    ...
    [<TestMethod>]
    member this.TestEx6 () =
        let result = run (exists .>> eof) """ex x in Real, y in pred, z in func (and (a,b,c))"""
        let actual = sprintf "%O" result
        printf "%O" actual
        Assert.IsTrue(actual.StartsWith("Success:"))
    ...
    member this.TestExN2 () =
        let result = run (existsTimesN .>> eof) """exn$1 x in Nat (not (iif ( true, not(false))))"""
        let actual = replaceWhiteSpace (sprintf "%O" result)
        printf "%O" actual
        Assert.IsTrue(actual.StartsWith("Success:"))
