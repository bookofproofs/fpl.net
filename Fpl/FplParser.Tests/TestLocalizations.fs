...
type TestLocalizations () =
    ...
    member this.TestEbnfTerm01 () =
        let result = run (ebnfTerm .>> eof) """x "\Leftrightarrow" y """
    ...
    member this.TestEbnfTransl01 () =
        let result = run (ebnfTransl .>> eof) """x "\Leftrightarrow" y """
        let actual = sprintf "%O" result
        printf "%O" actual
        Assert.IsTrue(actual.StartsWith("Success:"))
    ...
    member this.TestEbnfTransl03 () =
        let result = run (ebnfTransl .>> eof) """x "\Leftrightarrow" y | x "\Rightarrow" y """
        let actual = sprintf "%O" result
        printf "%O" actual
        Assert.IsTrue(actual.StartsWith("Success:"))
    ...
    member this.TestLocalization02 () =
        // comment
        let result = run (localization .>> eof) """loc iif(x,y) :=
    ...
    member this.TestLocalization03 () =
        let result = run (localization .>> eof) """loc NotEqual(x,y) :=
            !tex: x "\neq" y
            !eng: x "is unequal" y
